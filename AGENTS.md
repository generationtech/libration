...
11. Scene/map reference frame (Earth-fixed default; anchored Moon/Sun longitude-lock and position-lock) is independent of camera and of civil-time reference. Anchored frames target a trackable map object identity; target resolution is separate from frame math. Position-lock automatic scene-cover zoom is camera policy, not a frame transform and not camera-follow.
