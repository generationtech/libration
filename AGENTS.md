...
**Planning / discovery sessions:** after `README.md` and `ARCHITECTURE.md`, read **`PLAN.md` → “Agent session handoff (planning prompts)”** (scheduling snapshot table) and **`docs/ROADMAP.md`** phase status before recommending the next PR-sized slice. **Map substrates (shipped):** **`equirect-world-legacy-v1`** (default reference; bundled preview **closed**), **`equirect-world-topography-ne-v1`**, **`equirect-world-political-v1`**, **`equirect-world-geology-v1`** (non-transitional with bundled previews); **structured selector attribution** on all seven bundled families (`attribution`, optional `licenseNote`, `sourceLinks`; **Source & license** in `BaseMapStyleControl`). Legacy scene ids **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Scheduling:** **default macro PR track** = **Phase 8 / Slice 3** (queue **A**); **single best next PR** = next **A**-class substrate when sourced (queue **(2b)** month-aware selector polish **shipped**); **Slice 2** = composition *program* when **A** has no increment—fall through to **B**/**C**/**D**.
