...
- **Overlay readability stack (v1 + v1.1 + derived substrate lift + optional SceneConfig presentation + per-layer pilots):** derived upstream (RenderPlan hints + merged `cssFilter` where applicable); the shell attaches one `OverlayReadabilityFrame` per tick on `TimeContext` with subsolar veil, emissive **policy**, **substrate lift scale** (`substrateOverlayReadabilityLiftScale01` from effective base-map presentation + catalog `capabilities`), then applies normalized `scene.overlayReadability.presentation` (`readabilityVeilScale01`, `overlayLiftMultiplier01`) before hints. Optional **`perLayer` pilots** for default stack rows (`grid`, `solarAnalemma`, `subsolarMarker`, `sublunarMarker`, `cityPins`, `staticEquirectOverlay`) apply the same scalars again for those layers only. **Future:** readability pilots for additional custom stack rows (when product-defined) and richer substrate-only heuristics beyond presentation + catalog capabilities (see `PLAN.md` Slice 2).
