...
**Planning / discovery sessions:** after `README.md` and `ARCHITECTURE.md`, read **`PLAN.md` → “Agent session handoff (planning prompts)”** and **`docs/ROADMAP.md`** phase status before recommending the next PR-sized slice. **Map substrates:** **`equirect-world-topography-ne-v1`** (static Natural Earth–lineage topography), **`equirect-world-political-v1`** (Natural Earth–lineage political/reference substrate), and **`equirect-world-geology-v1`** (USGS public-domain–lineage geology substrate) are **shipped** in the bundled catalog (non-transitional; committed rasters + preview thumbnails); legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** remain resolver aliases for **`equirect-world-blue-marble-t-v1`**. **Scheduling:** “Primary active execution slice” in `PLAN.md` names the dominant **Slice 2** composition *program*; the **default next merged PR** is still **queue A** (Phase 8 / Slice 3) when a shippable catalog gap exists—**first inspect `public/maps` + catalog**: geology ship raster is **shipped**; default ordered **A** work is **narrow inventory polish** or the next sourced substrate unless blocked, then fall through to Slice 2 items **B**/**C**/**D** per the same handoff. **Slice 2** remains the primary composition *program* when **A** has no shippable increment or the session scopes illumination/readability work.
