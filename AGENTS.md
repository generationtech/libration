...
**Planning / discovery sessions:** after `README.md` and `ARCHITECTURE.md`, read **`PLAN.md` → “Agent session handoff (planning prompts)”** and **`docs/ROADMAP.md`** phase status before recommending the next PR-sized slice. **Map substrates:** **`equirect-world-topography-ne-v1`** (static Natural Earth–lineage topography) and **`equirect-world-political-v1`** (Natural Earth–lineage political/reference substrate; non-transitional in the bundled catalog) are **shipped** in the bundled catalog; **`equirect-world-geology-v1`** remains transitional until `public/maps/world-equirectangular-geology.jpg` is committed and validated; legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** remain resolver aliases for **`equirect-world-blue-marble-t-v1`**. **Scheduling:** “Primary active execution slice” in `PLAN.md` names the dominant **Slice 2** composition *program*; the **default next merged PR** is still **queue A** (Phase 8 / Slice 3) when the repo has a shippable catalog gap—**first inspect `public/maps`**: geology ship raster is the **default ordered** **A** gap unless blocked, then narrow inventory polish. **Slice 2** remains the primary composition *program* when **A** has no shippable increment or the session scopes illumination/readability work.
