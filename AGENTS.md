...
**Planning / discovery sessions:** after `README.md` and `ARCHITECTURE.md`, read **`PLAN.md` → “Agent session handoff (planning prompts)”** and **`docs/ROADMAP.md`** phase status before recommending the next PR-sized slice. **Map substrates (shipped static trio):** **`equirect-world-topography-ne-v1`**, **`equirect-world-political-v1`**, **`equirect-world-geology-v1`** (non-transitional with bundled previews—topography preview increment **closed**); **`equirect-world-legacy-v1`** (default reference; **no** catalog preview yet). Legacy scene ids **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Scheduling:** **Slice 2** = primary composition *program*; **default next merged PR** = **queue A** when a shippable gap exists—**first vertical:** legacy bundled preview (`PLAN.md` handoff item **3**); then attribution polish or next sourced substrate; fall through to **B**/**C**/**D** when **A** has no increment or session scopes Slice 2 only.
