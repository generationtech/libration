...
**Planning / discovery sessions:** after `README.md` and `ARCHITECTURE.md`, read **`PLAN.md` → “Agent session handoff (planning prompts)”** and **`docs/ROADMAP.md`** phase status before recommending the next PR-sized slice. **Map substrates (shipped static trio):** **`equirect-world-topography-ne-v1`** (ship raster; catalog **`previewThumbnailSrc` still missing**), **`equirect-world-political-v1`**, **`equirect-world-geology-v1`** (both non-transitional with bundled previews); legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Scheduling:** **Slice 2** = primary composition *program* (illumination + readability extensions); **default next merged PR** = **queue A** (Phase 8 / Slice 3) when a shippable gap exists—**default first vertical:** add topography bundled preview (`PLAN.md` handoff item **3**); then broader polish or next sourced substrate; fall through to **B**/**C**/**D** when **A** has no increment or session scopes Slice 2 only.
