...
- **Overlay readability stack (v1 + v1.1 + derived substrate lift + optional SceneConfig presentation + `grid` pilot per-layer):** derived upstream (RenderPlan hints + merged `cssFilter` where applicable); the shell attaches one `OverlayReadabilityFrame` per tick on `TimeContext` with subsolar veil, emissive **policy**, **substrate lift scale** (`substrateOverlayReadabilityLiftScale01` from effective base-map presentation + catalog `capabilities`), then applies normalized `scene.overlayReadability.presentation` (`readabilityVeilScale01`, `overlayLiftMultiplier01`) before hints. Optional **`scene.overlayReadability.perLayer.grid`** applies the same presentation scalars again for the lat/lon grid layer only. **Future:** per-layer readability for other stack ids and richer substrate-only heuristics beyond these axes (see `PLAN.md` Slice 2).
