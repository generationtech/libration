...
- **extended** composition-aware overlay readability: **`perLayer` pilots beyond the defaulted six ids** when new stack rows ship; **further** substrate modeling beyond the shipped presentation, sub-1 brightness dimming, and catalog flags (`overlayOptimized`, `darkFriendly`, `reliefShaded`, `boundaryDense`, …); **still** without backend policy.
