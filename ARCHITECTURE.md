...
- **incremental twilight transition tuning** (upstream only, `illuminationShading.ts`): cumulative constants-only refinements—smoother Gaussian coupling between anchor colors, cooler low-luminance terminator progression, capped atmospheric tint modulation, gentler day-side fade below +4° daylight clear—still one `rasterPatch`; no persisted twilight-softness controls in this slice.
