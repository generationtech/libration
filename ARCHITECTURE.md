...
- **Phase 8 base-map catalog:** **`equirect-world-legacy-v1`** (default reference) and static trio **`equirect-world-topography-ne-v1`**, **`equirect-world-political-v1`**, **`equirect-world-geology-v1`** are **shipped** with committed rasters + bundled **`previewThumbnailSrc`** for every bundled family (legacy thumb `world-equirectangular-thumb.jpg`, 800×400). **Structured attribution** (`attribution`, optional `licenseNote`, `sourceLinks`) is **shipped** in the map selector **Source & license** block (`BaseMapStyleControl`). **Default next** queue **A** PR: next sourced substrate or Slice 4 selector polish. See `docs/maps/MAP_ASSET_SOURCES.md`.
- live feeds.
