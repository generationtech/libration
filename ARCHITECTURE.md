...
**Consequence.** Clouds keep a per-source `coverageMask`, `qualityWeight`, and `cloudSignal`. Authority is: usable regional (`coverage && q>0`), then ring coverage, then extreme-geometry regional (`coverage && q==0`), then no data. The ring cannot reappear under usable q>0 regional coverage, including valid-clear. Quality=0 remains coverage and still paints when the ring is absent. Residual source-handoff contrast after a correct geometric winner is a presentation problem, not an authority defect. Do not blend overlapping observations to hide that mismatch. Provider display interpretation is [§3.9](#39-heterogeneous-display-rasters-are-normalized-before-shared-presentation).
