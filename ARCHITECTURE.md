...
- scene config (including `scene.illumination` and `scene.overlayReadability` — global `presentation` plus optional **`perLayer.grid`** pilot — for overlay legibility scaling upstream of RenderPlan hints).
...
- derived overlay readability (**v1 + v1.1 + substrate lift + optional SceneConfig presentation scaling + `grid` pilot per-layer scalars**, **milestone complete**): grid, analemma, subsolar/sublunar markers, city pins, and static equirect raster overlays (global combined veil → merged `imageBlit` cssFilter) scale resolved draw intent upstream (no backend composition policy). v1.1 adds emissive **policy** pressure only (no emissive texture sampling in the readability frame). **Substrate-aware lift scale** (derived from effective base-map presentation + catalog `capabilities`) modulates overlay css/stroke lift. **Persisted presentation:** `scene.overlayReadability.presentation` scales the derived combined veil and substrate lift in the shell (Layers tab). **Pilot:** optional `scene.overlayReadability.perLayer.grid` applies the same two scalars again for the lat/lon grid layer only. Production path: **one** `OverlayReadabilityFrame` per tick on `TimeContext` when the shell attaches it; layers call `getOverlayReadabilityFrameOrCompute` (fallback computes from `now` if omitted).
...
- **extended** composition-aware overlay readability: per-layer tuning beyond the shipped **`grid` pilot** (`scene.overlayReadability.perLayer.grid`); richer substrate modeling (e.g. per-family heuristics beyond presentation + catalog flags, still without backend policy).
