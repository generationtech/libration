...
**Consequence.** Do not implement entity-fixed mode by assigning camera centre to an entity each frame. Do not special-case the Moon in the camera. World wrapping for moving frames is a reference-frame concern: use continuous/unwrapped longitude rather than treating 180° → −179° as a new world. Earth-fixed identity remains the default production frame ([LIB-082](docs/work/LIB-082-scene-reference-frame-foundation.md)). Moon longitude-lock is the first non-Earth-fixed production frame ([LIB-083](docs/work/LIB-083-moon-longitude-locked-scene-frame.md)): longitude is locked, latitude remains canonical. Sun-fixed and generic entity-fixed remain later. Latitude lock on the same Moon kind is a later slice. North-up / no map rotation remains the posture of the current development phase; rotation would be a later camera question, not a reference-frame shortcut.

See [ADR 0026](docs/decisions/0026-scene-camera-independent-of-projection-and-reference-frame.md), [ADR 0027](docs/decisions/0027-moon-longitude-lock-is-a-scene-reference-frame.md), and [`docs/specs/scene/camera-and-reference-frame.md`](docs/specs/scene/camera-and-reference-frame.md).
