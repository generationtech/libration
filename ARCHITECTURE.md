...
- scene config (including `scene.illumination` and `scene.overlayReadability` — global `presentation` plus optional **`perLayer.grid`** / **`perLayer.solarAnalemma`** pilots — for overlay legibility scaling upstream of RenderPlan hints).
...
- **extended** composition-aware overlay readability: per-layer tuning beyond the shipped **`grid`** and **`solarAnalemma`** pilots; richer substrate modeling (e.g. per-family heuristics beyond presentation + catalog flags, still without backend policy).
