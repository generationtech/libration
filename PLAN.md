...
3. **Prioritized default queue for the *next* PR-sized slice** (inspect repo + catalog; pick the **first** item that is still a real, shippable gap; **one vertical per PR**):
   - **A. Phase 8 / Slice 3 (map inventory and scientific substrate expansion):** one bounded map-inventory / substrate increment aligned with `docs/ROADMAP.md` Phase 8 and **Slice 3** below (use existing `maps:prep` / bundled catalog patterns; do not invent new architecture). **Default next PR-sized slice (inspect repo + catalog):** **narrow inventory polish** (preview/selector/attribution/metadata consistency across shipped families; Slice 4 overlap, one vertical)—**or** the next **A**-class substrate onboarding when a new sourced raster and rights are ready (same workflow as topography/political/geology). **Shipped (rolling on this track):** **`equirect-world-topography-ne-v1`** (`world-equirectangular-topography.jpg`; **`reliefShaded`**); **`equirect-world-political-v1`** (`world-equirectangular-political.jpg`; Natural Earth attribution; non-transitional catalog entry; bundled preview thumbnail); **`equirect-world-geology-v1`** (`world-equirectangular-geology.jpg`; USGS public-domain lineage; non-transitional catalog entry; bundled preview thumbnail).
...
Status: **active (Phase 8)**—**default macro PR track** when handoff queue **A** applies; **advanced** this window: validated static global topography **`equirect-world-topography-ne-v1`** is **shipped** in the bundled catalog (`world-equirectangular-topography.jpg`; **`reliefShaded`** capability; legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** ids remain resolver aliases for **`equirect-world-blue-marble-t-v1`**); **`equirect-world-political-v1`** is **shipped** as a non-transitional Natural Earth–lineage political substrate (attribution + preview thumbnail in catalog); **`equirect-world-geology-v1`** is **shipped** as a non-transitional USGS public-domain–lineage geology substrate (`world-equirectangular-geology.jpg`; attribution + preview thumbnail in catalog). **Remaining** toward the same slice: further terrain or climate/vegetation substrates when sourced, emissive-compatible **substrate** families (distinct from the Black Marble composition-input path), narrow inventory polish—see `docs/ROADMAP.md` Phase 8.

Candidate work:

- narrow inventory polish (previews, attribution presentation, selector copy, placeholder hygiene across the young catalog—Slice 4 overlap).
