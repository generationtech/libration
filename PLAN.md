...
- static and month-aware base-map families (including **validated** static global topography **`equirect-world-topography-ne-v1`** and shipped Natural Earth–lineage political **`equirect-world-political-v1`**—non-transitional catalog entry with attribution and preview—in the bundled catalog; legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** ids remain resolver aliases for Blue Marble **T** month-aware topography).
...
The current strategic objective is **twofold**: (1) continue **Phase 8 / Slice 3** curated map inventory and scientifically grounded substrate validation where shippable gaps remain (see **Agent session handoff** queue **A** below—the **default next PR** when a real catalog gap exists; **inspect `public/maps`** first: the **first ordered** queue **A** gap is **`equirect-world-geology-v1`** ship raster + validation + placeholder clearance unless sourcing/legal blocks it, then narrow inventory polish); and (2) **extend** the delivered upstream planetary illumination and composition system (**additional** catalog/resolver substrate signals **beyond** the **shipped** eight-intrinsic overlay-lift contract (`reliefShaded` … `sunGlintDense`) and its presentation/dimming rules, **further atmospheric** refinement after cumulative shipped twilight tuning in `illuminationShading.ts`, **clouds/weather planning**) without destabilizing RenderPlan, SceneConfig authority, or execution-only backends. When queue **A** has no remaining shippable slice for the session (or is blocked), fall through to **B** / **C** / **D** in order unless the session explicitly scopes Slice 2 composition work.
...
3. **Prioritized default queue for the *next* PR-sized slice** (inspect repo + catalog; pick the **first** item that is still a real, shippable gap; **one vertical per PR**):
   - **A. Phase 8 / Slice 3 (map inventory and scientific substrate expansion):** one bounded map-inventory / substrate increment aligned with `docs/ROADMAP.md` Phase 8 and **Slice 3** below (use existing `maps:prep` / bundled catalog patterns; do not invent new architecture). **Default next PR-sized slice (inspect repo):** **`equirect-world-geology-v1`** onboarding—commit **`public/maps/world-equirectangular-geology.jpg`** (or adjust catalog `src` consistently), add preview + provenance, validate projection contract, then clear **`transitionalPlaceholder`** and sync `docs/maps/*` / roadmap handoff text—**unless geology is blocked** on sourcing or rights, in which case take the next **A**-class item (narrow preview/selector/attribution polish only; Slice 4 overlap, one vertical). **Shipped (rolling on this track):** **`equirect-world-topography-ne-v1`** (`world-equirectangular-topography.jpg`; **`reliefShaded`**); **`equirect-world-political-v1`** (`world-equirectangular-political.jpg`; Natural Earth attribution; non-transitional catalog entry; bundled preview thumbnail).
...
Status: **active (Phase 8)**—**default macro PR track** when handoff queue **A** applies; **partially advanced** this window: validated static global topography **`equirect-world-topography-ne-v1`** is **shipped** in the bundled catalog (`world-equirectangular-topography.jpg`; **`reliefShaded`** capability; legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** ids remain resolver aliases for **`equirect-world-blue-marble-t-v1`**); **`equirect-world-political-v1`** is **shipped** as a non-transitional Natural Earth–lineage political substrate (attribution + preview thumbnail in catalog). **Remaining** toward the same slice: geology onboarding (committed raster + validation; **`public/maps/world-equirectangular-geology.jpg`** still absent in default checkout), further terrain or climate/vegetation substrates when sourced, emissive-compatible **substrate** families (distinct from the Black Marble composition-input path), inventory polish—see `docs/ROADMAP.md` Phase 8.

Candidate work:

- geology onboarding (still open: catalog references **`/maps/world-equirectangular-geology.jpg`** but the file is not yet committed under `public/maps/` until sourced and validated).
...
- at least one additional scientifically grounded substrate family is validated and integrated cleanly (**partially met:** **`equirect-world-topography-ne-v1`**; geology and further families still count toward the same bar).
