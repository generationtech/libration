...
- static and month-aware base-map families (including **validated** static global topography **`equirect-world-topography-ne-v1`** in the bundled catalog; legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** ids remain resolver aliases for Blue Marble **T** month-aware topography).
...
2. **Primary active execution slice (composition *program*):** **Slice 2 — Planetary illumination — extensions on delivered foundations** (see near-term execution slices below)—the dominant medium-term **illumination + overlay-readability extension** program. Recent **doc-finalized** closures: overlay readability six-default `perLayer` stack, eight-intrinsic substrate lift contract, **second** narrow cumulative twilight pass in `illuminationShading.ts`. **This is not the same as “every next PR must be Slice 2 code”:** see item **3** for the **default next merged PR** when Phase 8 / Slice 3 still has a shippable catalog gap.
3. **Prioritized default queue for the *next* PR-sized slice** (inspect repo + catalog; pick the **first** item that is still a real, shippable gap; **one vertical per PR**):
   - **A. Phase 8 / Slice 3 (map inventory and scientific substrate expansion):** one bounded map-inventory / substrate increment aligned with `docs/ROADMAP.md` Phase 8 and **Slice 3** below (use existing `maps:prep` / bundled catalog patterns; do not invent new architecture). **Shipped (recent window):** validated static Natural Earth–lineage global topography **`equirect-world-topography-ne-v1`** (`world-equirectangular-topography.jpg`; catalog sets **`reliefShaded`** for upstream overlay lift; no new intrinsic keys); **`equirect-world-political-v1`** shipped raster + Natural Earth attribution + non-transitional catalog entry + preview thumbnail (`world-equirectangular-political.jpg`). **Still typical A gaps (repo-dependent):** geology family validation + committed ship raster (catalog references a missing asset until sourced); preview thumbnails, selector copy, or attribution polish where the catalog is thin (Slice 4 overlaps allowed when narrowly scoped to inventory).
...
Status: **active (Phase 8)**—**default macro PR track** when handoff queue **A** applies; **partially advanced** this window: validated static global topography **`equirect-world-topography-ne-v1`** is **shipped** in the bundled catalog (`world-equirectangular-topography.jpg`; **`reliefShaded`** capability; legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** ids remain resolver aliases for **`equirect-world-blue-marble-t-v1`**). **Remaining** toward the same slice: geology onboarding (committed raster + validation; runtime file still absent in default checkout), further terrain or climate/vegetation substrates when sourced, emissive-compatible **substrate** families (distinct from the Black Marble composition-input path), inventory polish—see `docs/ROADMAP.md` Phase 8.

Candidate work:

- geology onboarding (still open: catalog + docs may reference a raster not yet committed until sourced and validated).
