...
**Phase closure (runtime + docs aligned):** treat the seven intrinsics above plus `overlayOptimized` / `darkFriendly` multipliers and sub-1 brightness dimming as the **current shipped** substrate-readability catalog contract for overlay lift—not hypothetical.

**Next frontier (same subsystem, Slice 2):** additional optional `BaseMapCapabilities` axes or resolver-only signals **beyond** the shipped seven-intrinsic set (`reliefShaded` … `etchedReliefDense`) when product-justified; **still** no raster sampling unless explicitly scoped.
