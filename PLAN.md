...
| **Single best next PR** | **Next A-class substrate onboarding** when sourced | Queue **(2)** when raster + rights exist (preferred order: **climate normals**; land cover **`equirect-world-landcover-modis-v1`** and bathymetry **`equirect-world-bathymetry-etopo-v1`** **shipped**). **(2b)** month-aware selector **shipped**. If **(2)** is blocked, fall through to Slice 2 **B** then **C** only. |
