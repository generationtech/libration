...
- static and month-aware base-map families (including default reference **`equirect-world-legacy-v1`** with bundled preview, **validated** static global topography **`equirect-world-topography-ne-v1`**, shipped Natural Earth–lineage political **`equirect-world-political-v1`**, shipped USGS public-domain–lineage geology **`equirect-world-geology-v1`**, and shipped NOAA ETOPO 2022–lineage bathymetry **`equirect-world-bathymetry-etopo-v1`**—non-transitional catalog entries with **structured attribution** (`attribution`, optional `licenseNote`, `sourceLinks`) and bundled previews on all **eight** bundled families—in the bundled catalog; legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** ids remain resolver aliases for Blue Marble **T** month-aware topography).
...
The current strategic objective is **twofold**: (1) continue **Phase 8 / Slice 3** curated map inventory where shippable gaps remain (see **Agent session handoff** queue **A** below—the **default next merged PR** when a real gap exists is **next A-class substrate onboarding** when a sourced raster and rights are ready; **attribution presentation** and **month-aware selector polish** on queue **A** are **shipped**); and (2) **extend** the delivered upstream planetary illumination and composition system (**Slice 2** program: ninth+ optional catalog substrate signals, optional **third+** narrow twilight constants pass; weather/cloud **planning shipped**, implementation after Phase 10) without destabilizing RenderPlan, SceneConfig authority, or execution-only backends. When queue **A** has no remaining shippable increment for the session (no sourced substrate and no further inventory polish scoped), fall through to Slice 2 **B** then **C** (queue **D** weather/cloud **planning** is **shipped**—do not repeat doc-only planning PRs) unless the session explicitly scopes Slice 2 composition code.
...
**Scheduling snapshot (post–Phase 8 bathymetry closure; queue D weather/cloud planning closed):**
...
| **Single best next PR** | **Next A-class substrate onboarding** when sourced | Queue **(2)** when raster + rights exist (preferred order: vegetation/land cover → climate; bathymetry **`equirect-world-bathymetry-etopo-v1`** **shipped**). **(2b)** month-aware selector **shipped**. If **(2)** is blocked, fall through to Slice 2 **B** then **C** only. |
...
3. **Prioritized default queue for the *next* PR-sized slice** (inspect repo + catalog; pick the **first** item that is still a real, shippable gap; **one vertical per PR**):
   - **A. Phase 8 / Slice 3 (map inventory and scientific substrate expansion):** one bounded map-inventory / substrate increment aligned with `docs/ROADMAP.md` Phase 8 and **Slice 3** below (use existing `maps:prep` / bundled catalog patterns; do not invent new architecture). **Queue status:** **(1)** richer **attribution presentation** in the map selector — **shipped**; **(2b)** **Slice 4** month-aware selector polish (catalog copy, active UTC civil month for Blue Marble families, `productInstantMs` from render clock) — **shipped**; **(2)** static bathymetry/ocean substrate — **shipped** as **`equirect-world-bathymetry-etopo-v1`**. **Default next PR-sized slice:** **(2)** next **A**-class substrate onboarding when a sourced raster and rights are ready. **Preferred order when multiple candidates exist:** **vegetation/land cover**, then **climate normals** (bathymetry **shipped**). **Shipped (rolling on this track):** **`equirect-world-legacy-v1`** (default reference; bundled preview `world-equirectangular-thumb.jpg`); **`equirect-world-topography-ne-v1`** (`world-equirectangular-topography.jpg`; **`reliefShaded`**; bundled preview `world-equirectangular-topography-thumb.jpg`); **`equirect-world-political-v1`**, **`equirect-world-geology-v1`**, and **`equirect-world-bathymetry-etopo-v1`** (non-transitional; bundled previews; ETOPO hypsometry with −180…+180° equirect contract); **structured attribution** on all **eight** bundled families; **month-aware selector** copy + active month line in `BaseMapStyleControl`.
...
**Handoff when queue A (2) continues (next sourced substrate):** one PR — source raster + rights → `npm run maps:prep` (or GDAL/static export pipeline) → committed raster under `public/maps/` + preview → `base-map-catalog.json` row (`licenseNote` / `sourceLinks` as needed) → [`docs/maps/MAP_ASSET_SOURCES.md`](docs/maps/MAP_ASSET_SOURCES.md) provenance → catalog/resolver tests. **Preferred order:** vegetation/land cover, then climate normals (bathymetry/ocean **shipped** as **`equirect-world-bathymetry-etopo-v1`**).
...
Status: **active (Phase 8)**—**default macro PR track** when handoff queue **A** applies (**default next merged PR:** next **A**-class substrate when assets/rights exist—queue **A** item **(2)**; preferred order: vegetation/land cover → climate normals; bathymetry **shipped**). **Shipped** reference + static scientific substrates: **`equirect-world-legacy-v1`** (default reference; bundled preview), **`equirect-world-topography-ne-v1`**, **`equirect-world-political-v1`**, **`equirect-world-geology-v1`**, and **`equirect-world-bathymetry-etopo-v1`** (non-transitional where applicable; structured attribution + bundled previews on all **eight** bundled families). **Attribution presentation (Slice 3–4 overlap):** **closed**. **Month-aware selector polish (Slice 4):** **closed** (see closed month-aware increment). **Static trio preview polish:** **closed**. **Legacy reference preview:** **closed**. Legacy **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** ids remain resolver aliases for **`equirect-world-blue-marble-t-v1`**. **Remaining** toward the same slice: further sourced substrates (vegetation, climate), emissive-compatible **substrate** families (distinct from Black Marble composition input), optional selector label hygiene—see `docs/ROADMAP.md` Phase 8.
...
- at least one additional scientifically grounded substrate family is validated and integrated cleanly (**met and extending:** **`equirect-world-topography-ne-v1`**, **`equirect-world-political-v1`**, **`equirect-world-geology-v1`**, **`equirect-world-bathymetry-etopo-v1`**; further families—vegetation, climate—still count toward the same Phase 8 bar).
