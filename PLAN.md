...
**Scheduling snapshot (post–Slice 2 queues B/C closed; queue A (2) closed; queue D planning closed):**
...
| **Single best next PR** | **Sourced static substrate** (queue **A**) **or** explicitly scoped work | If queue **A** has no shippable increment and no explicit scope, **do not** default to ninth+ intrinsics or fourth+ twilight—state that composition baseline queues **B**/**C** are **closed**. |
...
2. **Primary active execution slice — two roles (do not conflate):**
   - **Default macro PR track (next merged PR):** **Phase 8 / queue A** when a **sourced static substrate** (raster + rights) or **explicitly scoped** map-inventory / Slice 4 polish is in the session—see table above. **Queue A (2) closed** until then. **Slice 2 queues B/C closed** as standing defaults (composition baseline complete). **All ten** bundled catalog families have `previewThumbnailSrc`, **structured selector attribution**, and **month-aware selector polish** (legacy reference preview + attribution + active UTC month line **closed**).
