...
**Scheduling snapshot (post–Phase 8 climate normals closure; queue D weather/cloud planning closed):**

| Role | Name | Meaning |
|------|------|---------|
| **Default macro PR track** | **Phase 8 / Slice 3** (queue **A**) when a shippable gap exists; else **Slice 2** **B**/**C** | What the **next merged PR** should advance unless session-scopes Slice 2 composition code only. |
| **Primary active execution slice** | **Phase 8 / Slice 3** (queue **A**) when a shippable gap exists; else **Slice 2** **B**/**C** | Map inventory when a sourced substrate or scoped polish remains; otherwise composition code—not Slice 2 micro-tuning while queue **A** still has a gap. |
| **Single best next PR** | **Slice 2 B** (ninth+ substrate intrinsic) or next sourced static substrate | Queue **(2)** and queue **C** third twilight pass **shipped**. **(2b)** month-aware selector **shipped**. Next: optional **B** when product note exists, or next sourced substrate when raster + rights exist; **C** fourth+ pass only when visually justified. |
...
Status: **baseline shipped (overlaps Phase 8 queue A)**—**attribution presentation** and **month-aware selector polish** (catalog copy, active UTC civil month line, render-clock `productInstantMs` in config UI) are **closed** for the current scope. **Default next work** on queue **A** is the **next sourced static substrate** when raster + rights exist (queue **(2)** land cover, bathymetry, and climate normals **shipped**); otherwise fall through to Slice 2 **B**/**C** or optional Slice 4 label hygiene when explicitly scoped.
