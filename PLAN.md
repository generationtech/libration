...
**Scheduling snapshot (Phase 10 active — next step `P10-3`; Phase 8/9 remaining deferred; Slice 2 queues B/C closed; queue A (2) closed; queue D weather planning closed; DLC consumers after `P10-7`):**
...
| **Single best next PR** | **Next pending Phase 10 step** (`P10-*`) | Implement the **first non-shipped** step in [`docs/specs/scene/dynamic-data-lifecycle-plan.md`](docs/specs/scene/dynamic-data-lifecycle-plan.md) (currently **`P10-3`**). One step per session. **Do not** ship user-facing dynamic overlays in Phase 10; **do not** default to Phase 8/9 or composition filler. |
...
3. Advance **Phase 10 / Slice 5** via sequenced `P10-*` steps in the lifecycle plan (currently **`P10-3`**).
...
**Active step:** `P10-3` (first pending step in the lifecycle plan table). Sessions implement **one** step only, then mark it shipped and advance this pointer.
