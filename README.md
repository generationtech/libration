...
- File-backed curated base-map catalog (optional **`capabilities`** for upstream overlay lift: **shipped** intrinsics **`reliefShaded`**, **`boundaryDense`**, **`chromaticDense`**, **`bathymetryShaded`**, plus **`overlayOptimized`** / **`darkFriendly`**).
