...
- **AI planning / discovery sessions:** after the docs above, read **`PLAN.md` → “Agent session handoff (planning prompts)”** for the repo’s **default macro PR track** (**Slice 2 / queue B** — ninth+ substrate readability intrinsic while queue **A (2)** is closed), **primary active execution slice** (same), and **single best next PR** (queue **B** unless a **sourced static substrate** re-opens queue **A** first). Land cover, bathymetry, climate normals **`equirect-world-climate-koppen-beck-v1`**, structured attribution, month-aware selector polish, **third** twilight pass, and **weather/cloud planning** ([`docs/specs/scene/weather-cloud-composition-plan.md`](docs/specs/scene/weather-cloud-composition-plan.md)) are **shipped**. Queue **A** re-opens for the next **sourced** static family or explicitly scoped catalog polish.
