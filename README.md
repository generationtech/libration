...
- File-backed curated base-map catalog (optional **`capabilities`** for upstream overlay lift: e.g. **`reliefShaded`**, **`boundaryDense`**, `overlayOptimized`, `darkFriendly`).
...
- Upstream twilight field: **shipped** incremental transition tuning in `src/renderer/illuminationShading.ts` (smoother anchor coupling, cooler terminator progression, gentler day-side atmospheric envelope; still one `rasterPatch`; non-emissive only).
