...
- File-backed curated base-map catalog (optional **`capabilities`** for upstream overlay lift: **seven shipped** intrinsic hints **`reliefShaded`**, **`boundaryDense`**, **`chromaticDense`**, **`bathymetryShaded`**, **`fineScaleTexture`**, **`labelDense`**, **`etchedReliefDense`**, plus presentation multipliers **`overlayOptimized`** / **`darkFriendly`**).
