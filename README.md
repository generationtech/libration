...
- **Derived overlay readability (v1 + v1.1, production-complete):** lat/lon grid, solar analemma ground track, subsolar/sublunar markers, and reference/custom **city pins** use `OverlayReadabilityHints` / per-pin `readabilityNightVeil01` from `OverlayReadabilityFrame` (subsolar veil; **v1.1** folds emissive **policy** from `scene.illumination.emissiveNightLights` into combined `readabilityVeil01At` / `globalReadabilityVeil01`; no emissive texture sampling in the readability path). **Static equirect raster overlays** use the global combined veil; `buildBaseRasterMapRenderPlan` merges bounded brightness/contrast with base-map-style presentation on the `imageBlit`. Vector stroke/alpha scale upstream (no dedicated readability SceneConfig keys). **One** frame per tick on `TimeContext`; layers use `getOverlayReadabilityFrameOrCompute`. **Substrate-aware lift (derived):** the shell passes resolved effective base-map presentation plus catalog `capabilities` into `computeOverlayReadabilityFrameFromTimeMs`; `substrateOverlayReadabilityLiftScale01` attenuates overlay css/stroke lift when the base map is already visually strong (no base-map pixel sampling).
