...
- [docs/specs/scene/dynamic-data-lifecycle-plan.md](docs/specs/scene/dynamic-data-lifecycle-plan.md) - **Phase 10 complete** dynamic data lifecycle contracts + `DLC-*` consumer tracker + **`DLU-*` live acquisition** tracker (lifecycle shipped in `src/lifecycle/`; **`DLC-1`**…**`DLC-4`** shipped; **Active `DLU-3`**; **`DLU-1`/`DLU-2` shipped**).
...
- **AI planning / discovery sessions:** after the docs above, read **`PLAN.md` → “Agent session handoff”** and **`docs/specs/scene/dynamic-data-lifecycle-plan.md`**. Sequenced post–Phase 10 **`DLC-1`…`DLC-4` complete**. **Default next:** **`DLU-*` live network acquisition** (**Active `DLU-3`**; paste DLU prompt in `PLAN.md`). Phase 10 lifecycle (`P10-0`…`P10-7`) is **complete**. **Composition baseline closed** (queues **B**/**C**). **Remaining Phase 8/9** deferred unless explicitly scoped. Weather/cloud **participation planning** shipped; Model A/B cloud **layers** shipped via `DLC-1`/`DLC-4`; live feeds via `DLU-*`.
