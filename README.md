...
- **Dynamic data lifecycle (Phase 10 complete) + `DLC-1`/`DLC-2`/`DLC-3` Model B consumers:** process-local lifecycle host (store / manager / resolver / acquisition / equirect + point-features + tracks materializers); SceneConfig rows `globalCloudsIr` (`global-clouds-ir-v1`), `earthquakes` (`usgs-earthquakes-v1`), and `orbitalTracks` (`iss-orbital-track-v1`); Layers toggles; periodic fixture acquisition outside rAF; overlays read sync-prepared views only (no fetch in render).
...
- [docs/specs/scene/dynamic-data-lifecycle-plan.md](docs/specs/scene/dynamic-data-lifecycle-plan.md) - **Phase 10 complete** dynamic data lifecycle contracts + `DLC-*` consumer step tracker (lifecycle shipped in `src/lifecycle/`; **`DLC-1`** / **`DLC-2`** / **`DLC-3`** shipped; next **`DLC-4`**).
...
- **AI planning / discovery sessions:** after the docs above, read **`PLAN.md` → “Agent session handoff”** and **`docs/specs/scene/dynamic-data-lifecycle-plan.md`**. **Default macro PR track:** post–Phase 10 **`DLC-*`** (next **`DLC-4`**; **`DLC-1`** clouds/IR + **`DLC-2`** earthquakes + **`DLC-3`** ISS tracks **shipped**). Phase 10 lifecycle (`P10-0`…`P10-7`) is **complete**. Use the DLC consumer prompt in `PLAN.md` to advance one step per session. **Composition baseline closed** (queues **B**/**C**). **Remaining Phase 8/9** deferred unless explicitly scoped. Weather/cloud **participation planning** shipped; further **implementation** is `DLC-*`.
