...
- **Derived overlay readability (v1 + v1.1):** lat/lon grid, solar analemma ground track, subsolar/sublunar markers, and reference/custom **city pins** use `OverlayReadabilityHints` / per-pin `readabilityNightVeil01` resolved upstream from `OverlayReadabilityFrame` (subsolar night veil; **v1.1** adds emissive **policy-only** legibility pressure from `scene.illumination.emissiveNightLights`, still no emissive texture sampling in the readability path). **Static equirect raster overlays** attach the global combined veil; upstream `buildBaseRasterMapRenderPlan` merges a bounded brightness/contrast CSS filter with any base-map-style presentation filter on the `imageBlit`. RenderPlan line/path/text weights and alphas scale for vectors (no dedicated readability keys in SceneConfig for v1.1; backend execution applies resolved filters only). The app shell attaches **one** `OverlayReadabilityFrame` per tick on `TimeContext` so layers reuse `getOverlayReadabilityFrameOrCompute` instead of each recomputing the frame from `now`.
