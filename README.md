...
- **Derived overlay readability (v1):** lat/lon grid, solar analemma ground track, and subsolar/sublunar markers carry upstream `OverlayReadabilityHints` from the same solar night-veil field as planetary illumination; RenderPlan line/path weights and alphas scale for legibility on the dark side (no new persisted SceneConfig; backend execution unchanged).
