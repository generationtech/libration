...
- Emissive night lights (NASA Black Marble–based composition raster) sampled upstream into that same planetary illumination raster, with presentation modes **Off / Natural / Enhanced / Illustrative** under Scene layers (default **Off**); durable `assetId` is catalog-backed and not surfaced as a base-map family. Layers also expose **intensity** and **faint-light lift** (driver exponent) tuning under `scene.illumination.emissiveNightLights.presentation`, persisted with the scene.
