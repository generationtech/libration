...
- **Derived overlay readability (v1):** lat/lon grid, solar analemma ground track, subsolar/sublunar markers, and reference/custom **city pins** use the same solar night-veil field as planetary illumination (`OverlayReadabilityHints` on vector payloads; per-pin `readabilityNightVeil01` on city pin rows). **Static equirect raster overlays** attach the global veil on the payload; upstream `buildBaseRasterMapRenderPlan` merges a bounded brightness/contrast CSS filter with any base-map-style presentation filter on the `imageBlit`. RenderPlan line/path/text weights and alphas scale for vectors (no new persisted SceneConfig; backend execution applies resolved filters only). The app shell may attach **one** `OverlayReadabilityFrame` per tick on `TimeContext` so layers reuse `getOverlayReadabilityFrameOrCompute` instead of each recomputing the frame from `now`.
