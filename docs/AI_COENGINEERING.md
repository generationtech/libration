...
**Map inventory (Phase 8 / Slice 3):** The bundled base-map catalog (`src/assets/maps/base-map-catalog.json`) remains authoritative; do not runtime-scan `public/maps`. **Shipped families** (including legacy reference preview—**closed** increment): **`equirect-world-legacy-v1`**, **`equirect-world-topography-ne-v1`**, **`equirect-world-political-v1`**, **`equirect-world-geology-v1`**. **Legacy scene ids:** **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Default next PR:** **`PLAN.md` → Agent session handoff queue A** — **(1)** attribution polish; **(2)** next sourced substrate; fall through to Slice 2 **B**/**C**/**D** when **A** has no increment.
