...
**Map inventory (Phase 8 / Slice 3):** The bundled base-map catalog (`src/assets/maps/base-map-catalog.json`) remains authoritative; do not runtime-scan `public/maps`. **Shipped:** every bundled family has **`previewThumbnailSrc`** and structured selector attribution (`attribution`, optional `licenseNote`, up to two `sourceLinks`); **Source & license** block in `BaseMapStyleControl` (catalog-only—not SceneConfig). **Shipped static scientific substrates:** topography **`equirect-world-topography-ne-v1`**, political **`equirect-world-political-v1`**, geology **`equirect-world-geology-v1`**, bathymetry **`equirect-world-bathymetry-etopo-v1`** (ETOPO 2022; enforce −180…+180° equirect contract—see `docs/maps/MAP_ASSET_SOURCES.md` dateline-roll steps; `bathymetryOnboardedAsset.test.ts` guards registration). **`maps:prep`** emits primary `attribution` only; curators add `licenseNote` / `sourceLinks` in catalog JSON when onboarding. **Legacy scene ids:** **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Default macro PR track / single best next PR:** **`PLAN.md` → Agent session handoff** — next **A**-class substrate when sourced (**vegetation/land cover → climate**; bathymetry **shipped**; month-aware selector polish **shipped**); fall through to Slice 2 **B**/**C** when queue **A** has no shippable increment (queue **D** planning **shipped**).
