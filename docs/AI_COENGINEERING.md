...
**Map inventory (Phase 8 / Slice 3):** The bundled base-map catalog (`src/assets/maps/base-map-catalog.json`) remains authoritative; do not runtime-scan `public/maps`. **Shipped static trio:** **`equirect-world-topography-ne-v1`**, **`equirect-world-political-v1`**, **`equirect-world-geology-v1`** (non-transitional; attribution + bundled preview thumbnails where catalog lists them—see **`PLAN.md` closed increments**). **Legacy scene ids:** **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Default next PR:** **`PLAN.md` → Agent session handoff queue A** — broader polish or next sourced substrate; fall through to Slice 2 **B**/**C**/**D** when **A** has no increment.
