...
**Map inventory (Phase 8 / Slice 3):** The bundled base-map catalog (`src/assets/maps/base-map-catalog.json`) remains authoritative; do not runtime-scan `public/maps`. **Shipped:** every bundled family has **`previewThumbnailSrc`** and structured selector attribution (`attribution`, optional `licenseNote`, up to two `sourceLinks`); **Source & license** block in `BaseMapStyleControl` (catalog-only—not SceneConfig). **`maps:prep`** emits primary `attribution` only; curators add `licenseNote` / `sourceLinks` in catalog JSON when onboarding. **Legacy scene ids:** **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Default macro PR track / single best next PR:** **`PLAN.md` → Agent session handoff** — next **A**-class substrate when sourced (month-aware selector polish **shipped**: active UTC civil month in `BaseMapStyleControl`, render-clock `productInstantMs`); fall through to Slice 2 **B**/**C**/**D** when queue **A** has no shippable increment.
