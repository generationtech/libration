...
**Map inventory (Phase 8 / Slice 3):** The bundled base-map catalog (`src/assets/maps/base-map-catalog.json`) remains authoritative; do not runtime-scan `public/maps`. **Shipped:** **`equirect-world-topography-ne-v1`** (Natural Earth–lineage raster; **`reliefShaded`**; ship raster committed; **catalog preview still missing**); **`equirect-world-political-v1`** and **`equirect-world-geology-v1`** (non-transitional; attribution + bundled preview thumbnails; geology onboarded this window—see **`PLAN.md` closed geology increment**). **Legacy scene ids:** **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Default next PR:** **`PLAN.md` → Agent session handoff queue A** — **first vertical:** topography bundled preview; then broader polish or next sourced substrate; fall through to Slice 2 **B**/**C**/**D** when **A** has no increment.
