...
**Map inventory (Phase 8 / Slice 3):** The bundled base-map catalog (`src/assets/maps/base-map-catalog.json`) remains authoritative; do not runtime-scan `public/maps`. **Shipped:** every bundled family has **`previewThumbnailSrc`** and structured selector attribution (`attribution`, optional `licenseNote`, `sourceLinks`); **Source & license** block in `BaseMapStyleControl`. **Legacy scene ids:** **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Default macro PR track / single best next PR:** **`PLAN.md` → Agent session handoff** — next sourced substrate or Slice 4 selector polish; fall through to Slice 2 **B**/**C**/**D** when **A** has no increment.
