...
**Map inventory (Phase 8 / Slice 3):** The bundled base-map catalog (`src/assets/maps/base-map-catalog.json`) remains authoritative; do not runtime-scan `public/maps`. **Shipped:** every bundled family has **`previewThumbnailSrc`** (legacy reference preview **closed**—`world-equirectangular-thumb.jpg` on **`equirect-world-legacy-v1`**). **Legacy scene ids:** **`equirect-world-topography-v1`** / **`equirect-world-topo-v1`** → **`equirect-world-blue-marble-t-v1`**. **Default macro PR track / single best next PR:** **`PLAN.md` → Agent session handoff** — **richer attribution presentation** in map selector (queue **A** item **(1)**); then next sourced substrate; fall through to Slice 2 **B**/**C**/**D** when **A** has no increment.
