...
**Planning tie-break:** read **`PLAN.md` → Agent session handoff** and [`docs/specs/scene/dynamic-data-lifecycle-plan.md`](specs/scene/dynamic-data-lifecycle-plan.md). **Default next** = first pending **`P10-*`** step (currently **`P10-4`**); one step per session via the Phase 10 prompt in `PLAN.md`. Phase 10 = lifecycle only (no user-facing dynamic overlay). Post–`P10-7` → **`DLC-*`** consumers. Do not default to Phase 8/9 or composition filler while Phase 10 is open.
