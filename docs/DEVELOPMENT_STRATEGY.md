...
**Planning tie-break:** read **`PLAN.md` → Agent session handoff** and [`docs/specs/scene/dynamic-data-lifecycle-plan.md`](specs/scene/dynamic-data-lifecycle-plan.md). Phase 10 lifecycle **complete**; **`DLC-1`** / **`DLC-2`** / **`DLC-3`** shipped. **Default next** = first pending **`DLC-*`** step (currently **`DLC-4`**); one vertical per session via the DLC consumer prompt in `PLAN.md`. Do not default to Phase 8/9 or composition filler while early DLC is open.
