...
**Planning tie-break:** read **`PLAN.md` → Agent session handoff** and [`docs/specs/scene/dynamic-data-lifecycle-plan.md`](specs/scene/dynamic-data-lifecycle-plan.md). Phase 10 lifecycle **complete**; **`DLC-1`**…**`DLC-4`** shipped (sequenced table complete). Further `DLC-*` or Phase 8/9 resume only with **explicit scope**; one vertical per session via the DLC consumer prompt in `PLAN.md` when scoped. Do not invent filler composition PRs.
