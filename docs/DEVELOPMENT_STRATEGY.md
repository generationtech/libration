...
**Planning tie-break:** for “what PR next” without extra steering, read **`PLAN.md` → “Agent session handoff (planning prompts)”** (queue **A** = default map PR when a gap exists—**first:** attribution polish or next sourced substrate; all bundled family previews **shipped**; **Slice 2** = composition *program* when **A** has no increment, is blocked, or session scopes composition only).
