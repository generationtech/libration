...
- when a vertical phase ships (e.g. overlay readability **v1 + v1.1** + **derived substrate lift scale** + **substrate heuristic increments** (catalog `reliefShaded` / `boundaryDense` / `chromaticDense` / `bathymetryShaded` / `fineScaleTexture` / `labelDense` / `etchedReliefDense`, sub-1 brightness dimming), **`scene.overlayReadability.presentation` scalars**, **six default-stack `scene.overlayReadability.perLayer` pilots** per `SCENE_OVERLAY_READABILITY_PER_LAYER_PILOT_KEYS`, or **incremental twilight transition tuning** in `illuminationShading.ts`), update `PLAN.md`, `docs/ROADMAP.md`, `ARCHITECTURE.md` maturity, `docs/FUTURE_FEATURES.md`, `docs/PROJECT_STRATEGY.md`, `docs/AI_COENGINEERING.md`, `AGENTS.md`, `docs/maps/MAP_ASSET_SOURCES.md`, `docs/maps/MAP_ASSET_STRATEGY.md`, and `.cursor/rules/050-docs-and-roadmap.mdc` so shipped vs backlog is obvious without reading source.
