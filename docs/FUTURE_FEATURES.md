...
- Additional trackable targets beyond Moon, Sun, and ISS (later cities/planets if they fit), only if a candidate satisfies the trackability contract in [ADR 0032](decisions/0032-anchored-frames-target-a-trackable-map-object.md). ISS tracking is implemented ([LIB-089](work/LIB-089-iss-tracking-target.md), [ADR 0033](decisions/0033-iss-tracking-reuses-anchored-frame-target-architecture.md)). Not a generic picker. Earthquakes remain excluded from tracking.
- Split the transitional seven-choice Scene frame selector into Tracking target + Tracking mode, and later direct selection of rendered objects (click-to-track). Polarized UX work, not authorized by LIB-089.
