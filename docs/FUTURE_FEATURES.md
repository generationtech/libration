...
- **Overlay readability (v1 + v1.1 + derived substrate lift):** `OverlayReadabilityFrame` on `TimeContext` (one per tick from the shell): **v1** subsolar night veil on grid, analemma, subsolar/sublunar markers, city pins, and static equirect rasters; **v1.1** adds deterministic emissive **policy-only** legibility pressure from `scene.illumination.emissiveNightLights` into `readabilityVeil01At` / `globalReadabilityVeil01` (no emissive texture sampling in the readability path); **substrate lift scale** on the same frame (`substrateOverlayReadabilityLiftScale01`) from effective base-map presentation + catalog `capabilities`, carried into hints / city pins / merged `cssFilter`; vector stroke/alpha scaling resolved upstream.
...
- overlay readability **beyond derived v1.1 + substrate lift scale:** optional dedicated SceneConfig presentation axis for readability; per-layer tuning where justified.
