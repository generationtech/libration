...
**Candidate.** Strong follow-on / enabling feature. Related to, but not the same as, the Eclipse System. Lunar eclipse presentation no longer paints the geometric *current-instant* Moon-above-horizon contour ([LIB-046](work/LIB-046-remove-lunar-eclipse-moon-visible-geography.md)); illumination still uses that geometry internally (`lunarDot ≥ 0`). [LIB-054](work/LIB-054-static-lunar-eclipse-visibility-footprint.md) adds an *event-static* visibility footprint for lunar eclipses only. A standing ambient Lunar Visibility overlay remains unapproved.
...
Weather beyond the current-observation IR Clouds foundation remains unapproved. Future radar, lightning, wind analysis, and tropical/severe products inherit the same doctrine ([ADR 0023](decisions/0023-observational-composites-heterogeneous-observation-times.md)): most recent valid observation beats artificial timestamp synchronization; domains must not wait on one another; each keeps its own observation time and freshness. Do not implement them here. Do not start another IR Clouds corrective item from this list.

- polar LEO gap-fill (the geostationary ring leaves polar holes; do not treat holes as clear sky)
- Clouds overlap feathering in dual-coverage **q>0** interiors (not at coverage/no-data limbs, not at q=0 GEO skirts). [LIB-075](work/LIB-075-weather-5-3-1-ring-component-geometry-quality.md) refined ring-vs-q0 using component geometry. Remaining NATL East/MSG and Pacific West/Himawari handoffs are usable-quality crossovers; do not start a blend LIB automatically. Dual q>0 MSG∩Himawari over India remains empty after LIB-079; do not start q>0 blending as the next step.
- Inherent IR Clouds limits after [LIB-079](work/LIB-079-weather-5-5-1-ring-canonical-identity-grayscale.md) identity grayscale: warm/low-cloud ambiguity, cold-surface / polar ice, GIBS false-color convective cores versus ring gray, residual provider texture. These are not a missing black-point and do not automatically justify blending, polar special cases, or another corrective calibration LIB.
...
- Additional trackable targets beyond Moon, Sun, and ISS (later cities/planets if they fit), only if a candidate satisfies the trackability contract in [ADR 0032](decisions/0032-anchored-frames-target-a-trackable-map-object.md). ISS tracking is implemented ([LIB-089](work/LIB-089-iss-tracking-target.md), [ADR 0033](decisions/0033-iss-tracking-reuses-anchored-frame-target-architecture.md)). Tracking target + Tracking mode UX is implemented ([LIB-090](work/LIB-090-tracking-target-and-mode-ux-foundation.md), [ADR 0034](decisions/0034-tracking-ui-is-orthogonal-target-and-mode.md)). Direct click-to-track for Moon, Sun, and ISS is implemented ([LIB-091](work/LIB-091-direct-click-to-track-for-map-objects.md), [ADR 0035](decisions/0035-click-to-track-uses-scene-space-semantic-hit-targets.md)). Not a generic picker. Earthquakes remain excluded from tracking.
- Direct selection of further rendered objects (city pins, planets) using the existing `setTrackingTarget` seam once those identities are production trackable targets. Milky Way is a band plus Galactic-center/anticenter glyphs, not a single canonical point; tracking it needs a later semantic design. Polarized UX work, not authorized by LIB-091.
