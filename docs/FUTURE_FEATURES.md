...
- bundled catalog **`capabilities`** consumed upstream for overlay lift (optional intrinsics include **`reliefShaded`**, **`boundaryDense`**, **`chromaticDense`**, **`bathymetryShaded`**, **`fineScaleTexture`**, **`labelDense`**, **`etchedReliefDense`**—curator metadata; e.g. Blue Marble TB sets **`bathymetryShaded`** with **`reliefShaded`**; Blue Marble **BM**/**T** set **`fineScaleTexture`**; political/geology set **`labelDense`** alongside chromatic/boundary hints; legacy world sets **`etchedReliefDense`** for packaged etched shaded relief).
...
- configurable twilight softness (not persisted in the shipped incremental tuning pass; future SceneConfig axis if product adds it).
