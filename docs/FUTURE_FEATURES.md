...
- Additional trackable targets beyond Moon and Sun (ISS first; later cities/planets if they fit), only if a candidate satisfies the trackability contract in [ADR 0032](decisions/0032-anchored-frames-target-a-trackable-map-object.md). ISS would add a stable identity, resolve its existing canonical lon/lat, and expose UI — not another transform/camera refactor. Not a generic picker. Earthquakes remain excluded from tracking.
