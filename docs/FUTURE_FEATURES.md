...
- Additional trackable targets beyond Moon, Sun, ISS, city pins, and eligible planetary current glyphs, only if a candidate satisfies the trackability contract in [ADR 0032](decisions/0032-anchored-frames-target-a-trackable-map-object.md). City and planet tracking is implemented ([LIB-092](work/LIB-092-city-and-planet-tracking-targets.md), [ADR 0036](decisions/0036-city-and-planet-tracking-reuses-structured-target-identity.md)). Not a generic picker. Earthquakes remain excluded from tracking.
- Milky Way tracking. Current rendering is a galactic-plane band plus tagged Galactic Center and Anticenter subpoints, not a single canonical tracking point. Do not invent one. A later item may choose Center, Anticenter, or another explicit point identity. Not authorized by LIB-092.
