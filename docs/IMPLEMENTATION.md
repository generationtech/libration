...
- **Event information.** Layers inspectable surface for the presented upcoming solar, upcoming lunar, active solar, or active lunar event: accessible type name, UTC date, greatest-eclipse UTC, lifecycle, product-time relative countdown, and a compact geography legend. The primary event is active solar, then active lunar, then the nearest upcoming of either kind. Reference-city circumstances reuse the E4 block when they belong to the same presented event. Hidden when no relevant event exists.
- **Labels.** At most one restrained map label for that same primary event. `scene.eclipseInfo.labelsEnabled` is captured into the solar/lunar layers at construction; toggling it rebuilds the layer registry so the map label appears or disappears immediately. Event information and persistent chrome status are independent and do not require a registry rebuild.
...
Post-E6 reconciliation ([LIB-020](work/LIB-020-eclipse-reconciliation-and-lunar-forecast.md)) added lunar forecasting on the existing EclipseAuthority / EclipseEventService path. It is not an E7 program phase. No new ADR: the existing authority, forecast-service, and global-first decisions already cover it.
