...
| Tab | Owns |
|-----|------|
| Layers | Scene stack toggles (Layer masters topic, default): lunar ground track, lunar locus, Solar eclipses, Lunar eclipses, and the other overlay masters. Internal **Layers topic** selector (UI-only, not persisted; inactive topics unmount; compact selector uses shared `.config-topic-nav` `position: sticky` inside the existing `.config-tab-panel` scroller, not the viewport; changing topic resets that panel’s `scrollTop` and does not call `updateConfig`): Map (family, preview, attribution, presentation); Illumination (moonlight, night lights, cloud participation); Eclipse System (event information / labels; solar and lunar forecast horizons and type filters; Moon Earth-shadow; alignment; reference-city details/chrome; independent appearance; active solar eclipse shading enable and Subtle/Normal/Dramatic intensity; **Eclipse Tour** at the bottom — Demo-time navigation, not a second clock); Moon & libration (size, ring/crosshair, map/observer orientation, use-reference-city, color, thickness, motion scale); Astronomy paths (past/future track extents and stroke colors; independent Lunar locus and Solar analemma stroke color/thickness); **Space objects** (ISS presentation: orbit track, past/future segments, minute/orbit horizons, colors, thickness, glyph, size, label; **Planets**: Mercury–Neptune plus Pluto body enable/color/locus plus shared current-subpoint, glyph, ground-track, and locus style; **Milky Way**: zenith-projection Galactic plane / approximate band / ribs / Galactic center, not naked-eye visibility); Advanced (overlay-readability presentation and per-layer pilots). Optional live overlays (clouds/IR, earthquakes, ISS) remain Layer masters checkboxes. Planets and Milky Way are also Layer masters (offline astronomy, factory off). Live eclipse event rows live on the map, not in this tab. |
...
Eclipse Tour is a domain sequencer **above** Demo time, not a second clock ([ADR 0015](decisions/0015-domain-tour-sequencer-drives-shared-demo-time.md)). Controls live at the bottom of Layers → Eclipse. Data/Demo stays domain-neutral.

Durable `scene.eclipseTour` stores range, solar/lunar families, loop, lead-in, and post-wait. Playback speed is `data.demoTime.speedMultiplier` only. Runtime phase, event index, and owned Demo start ISO are session-only and start inactive.

On Start, the tour enumerates bundled catalog events (type filters included; layer masters ignored), sets Demo mode/enabled/start to the current event’s clamped lead-in, and resumes Demo playback. At each event’s clamped post-wait it jumps Demo start to the next lead-in. Loop wraps; loop off pauses at the final allowed instant. Stop deactivates sequencing and leaves product time paused. Reset returns to the **current event** lead-in (the Demo configured start), not the tour-range start.

Manual Data Demo start edits and Demo becoming inactive deactivate the tour. Pause and speed from either surface stay shared. Forecast horizons and eclipse presentation are not overridden. Catalog enumeration is not performed every frame.

**Config-panel time:** while the panel is open, `configPanelProductInstantMs` tracks the render clock. It updates when the **UTC calendar month** changes (month-aware base-map selector) or when product time jumps by **≥ 1 s** (Demo seeks, including Go to next Prime). That avoids per-frame Config re-renders while still keeping viewing-window status on the same product instant as the scene.
