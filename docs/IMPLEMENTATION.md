...
| Solar eclipses | `solarEclipseLayer.ts` | NASA-derived solar overlay (E1 live footprint + E2 forecast corridor + E5 alignment beam + E6 labels/styles). Default **on**: geography appears only while an event is relevant. Forecast horizon (`0` / live only, `1`, `3`, `7`, `14`, `30`, `90`, `365` days; default **7**) is how early upcoming solar geography appears, not eclipse duration. Upcoming events emit a quieter event-path corridor (cached, time-independent) and a representative greatest-eclipse partial region. Active events emit the live E1 umbra/antumbra and partial footprint, and may retain the corridor as context when the horizon is not live-only. Partial-only events never fabricate a central corridor. An optional active-only alignment ribbon (E5) connects the Sun/Moon glyph cluster to the live umbra/antumbra; partial-only events get a local bloom, not a fabricated target. Presentation-only type filters (total / annular / partial / hybrid) default **on**. Child geography toggles and user style (color / thickness / opacity) persist on the row; defaults preserve the verified E1–E5 look. Alignment is `scene.eclipseAlignment`. Canvas sees no eclipse astronomy. |
| Lunar eclipses | `lunarEclipseLayer.ts`, Earth-shadow fields on `sublunarMarkerLayer.ts` | NASA-derived lunar overlay (E3 + E5 axis + E6 labels/styles). Default **on**: geography appears only while an event is relevant. Active events only: no lunar forecast map. The layer emits the terrestrial Moon-above-horizon region and geometric horizon boundary, plus an optional Earth-shadow alignment axis toward the Moon glyph. Earth-shadow treatment on the Moon glyph is a clipped overlay inside the existing Moon RenderPlan (`earthShadowOverlay`), after phase shading and before the libration mark. Presentation-only type filters (total / partial / penumbral) default **on**. Child controls and user style persist on the row. Alignment is `scene.eclipseAlignment`. Canvas sees no eclipse astronomy. |
...
- **Lookup:** `activeLunarEclipseAt`, `getLunarEclipseEventById`, `nextLunarEclipseAfter`, and `lunarEclipsesIntersecting` use binary search. E3 presentation is **active-only** (P1/first contact through P4/last contact). No lunar forecast UI.
...
- **Moon glyph:** existing size, phase, optical libration, and observer orientation are unchanged. Eclipse overlay is independent of phase shading. Draw order: glow → phase disc → phase shadow → Earth-shadow fills → libration mark → outline.
- **Visibility region:** geometric lunar altitude = 0 on a spherical Earth (same unit-sphere dot product as illumination `lunarDot`). No refraction. The boundary is a great-circle horizon contour around the current ambient `sublunarPoint`. Not a solar-style corridor. Equatorial moons use a 180° longitude strip sampled so dateline unwrap does not invert the hemisphere.
...
- **Solar.** For total/annular events with a live central point, a tapered translucent ribbon runs from the Sun/Moon glyph cluster (ambient midpoint) to the live umbra (total) or antumbra (annular). Partial-only events, and central events before/after the umbra/antumbra is on Earth, get a local alignment bloom around the glyphs — no fabricated terrestrial target. The beam is not aimed along the forecast corridor.
...
- **Layering.** Alignment bands draw after the live partial region and before the live umbra/antumbra, so the footprint remains the geographic target. Glyphs stay above the eclipse layers. Moon-over-Sun overlap is unchanged.
...
Production surface (E6, [LIB-019](work/LIB-019-eclipse-product-polish.md)). E6 does not add astronomy. It groups the E1–E5 controls, adds inspectable event information, restrained labels, presentation-only type filters, user styling, honest unsupported-range copy, and reviewed defaults.
...
- **Event information.** Layers inspectable surface for the presented upcoming solar, active solar, or active lunar event: accessible type name, UTC date, greatest-eclipse UTC, lifecycle, product-time relative countdown, and a compact geography legend. Reference-city circumstances reuse the E4 block when they belong to the same presented event. Hidden when no relevant event exists. Upcoming solar details are available before first contact.
- **Labels.** At most one restrained map label: nearest upcoming solar (greatest-eclipse point) or the active solar/lunar event. No per-polygon labels. Off when `scene.eclipseInfo.labelsEnabled` is false.
...
- **Config groups** in Layers: Eclipses (information / labels); Solar (horizon, types, live/forecast geography); Lunar (types, Moon-shadow, Moon-visible boundary/region); Alignment; Reference city; Eclipse appearance. Child controls disable when the parent layer or master is off. Forecast corridor/partial disable when the horizon is Live only.
- **No lunar forecast map.** Active lunar only. `nextLunarEclipseAfter` remains an authority helper, not a product forecast UI.
...
| Tab | Owns |
|-----|------|
| Layers | Scene stack toggles (including lunar ground track, lunar locus, Solar eclipses, and Lunar eclipses), grouped Eclipse System controls (event information / labels; solar forecast horizon and type filters; lunar type filters and Moon-visible geography; alignment; reference-city details/chrome; independent appearance), inspectable event information plus local circumstances, Moon size and libration presentation (ring/crosshair, map/observer orientation, use-reference-city, color, thickness, motion scale), independent Lunar locus and Solar analemma stroke color/thickness, past/future track extents and stroke colors, illumination (moonlight, emissive night lights, cloud participation), overlay-readability presentation, optional live overlays (clouds/IR, earthquakes, ISS) |
