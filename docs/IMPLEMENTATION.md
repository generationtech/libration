...
- `src/lifecycle/dynamicCloudOpacityMaterializer.ts` — Model A display-luma → cloud-opacity attenuation. Clouds v1 forces `cloudParticipationMode: "off"` at the overlay factory; the Illumination topic no longer exposes the control. Enabling Clouds does not change the illumination raster. The materializer remains for tests and possible future optical-depth products.
...
Acquisition is periodic and runs on an injectable timer, never inside `requestAnimationFrame`, a layer constructor, or a RenderPlan builder. Refresh cadence is per source: 10 minutes for Clouds (GIBS Band13 slot cadence), 5 minutes for earthquakes, 2 hours for the ISS TLE (CelesTrak GP update cycle). Enabling a consumer calls `startPeriodic` with `runImmediately: true`, so the first fetch does not wait for the interval tick. ISS re-enable with a still-ready in-memory snapshot starts the interval without an immediate re-download. ISS TLE refresh is background maintenance; the marker moves every frame via local SGP4(product UTC). Clouds GetMap and GetCapabilities attempts are bounded at 15 seconds. Earthquake USGS attempts are bounded at 15 seconds so a hung feed can leave Loading and resolve to stale or unavailable.
...
Three live feeds are wired, each classified `timePolicy: "wallClockCurrent"` under their present implementations. Clouds, earthquakes, and ISS do not present fixture as live: a failed live fetch with no usable live snapshot leaves the overlay unavailable. Last-good **live** Clouds may remain as stale while observation age is ≤ 6 h. ISS HTTP attempts are bounded at 8 seconds; timeout is not treated as a user abort, so the secondary provider can run in the same cycle. After all-provider failure, a single 5-minute retry is scheduled if ISS remains enabled, then the 2-hour cadence. [ADR 0014](decisions/0014-iss-live-tle-ordered-provider-failover.md).

**Clouds v1.** Durable `sourceId` remains `global-clouds-ir-v1`; the user-facing layer is **Clouds**. Live authority is NASA GIBS WMS 1.1.1 stacking GOES-West, GOES-East, and Himawari Band 13 Clean Infrared (`LAYERS` bottom→top: West, East, Himawari) as a transparent PNG 2048×1024 full-world GetMap. `TIME` is always explicit: GetCapabilities layer defaults are read, the earliest of the three defaults is the common stack TIME, then GetMap walks back 10-minute slots (max 18) until a usable mosaic is found. `snapshot.validTimeMs` is that mosaic TIME (observation). `snapshot.acquiredAtMs` is fetch time. Freshness uses observation age: ≤3 h recent, >3 h and ≤6 h stale, >6 h suppress. Coverage is partial (Africa / Europe / polar holes stay transparent, not clear sky). Bytes are decoded outside rAF and converted with an approximate Rec.601 luma **smoothstep 100→195** to cool-white RGBA `(248, 250, 252)` (`cloudHighlightTransfer.ts`) before the equirect materializer. Factory overlay opacity is **0.42**; user opacity multiplies derived alpha. The host keeps about **4** in-memory Clouds snapshots. Layers → **Weather** (after Earthquakes, before Advanced) holds Cloud opacity and status only. [ADR 0022](decisions/0022-observational-data-three-clocks.md).

| Durable `sourceId` | Feed | When product time is not live-enough |
|--------------------|------|--------------------------------------|
| `global-clouds-ir-v1` | NASA GIBS WMS Band13 GOES-East + GOES-West + Himawari PNG stack (explicit `TIME`) | Overlay suppressed (Clouds remain current-only; no historical GIBS TIME query) |
...
Physical cloud illumination participation is **off** for Clouds v1. Enabling the Clouds overlay arms acquisition; a stored `cloudParticipation` mode does not. The overlay factory forces `cloudParticipationMode: "off"`.
