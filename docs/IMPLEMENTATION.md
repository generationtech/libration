...
The **scene/map reference frame** (`src/core/sceneReferenceFrame.ts`) is a transform *before* projection, carried on `SceneRenderInput.sceneReferenceFrame`. Production kinds: Earth-fixed identity (default; exact short-circuit so LIB-081 numbers are unchanged); Moon longitude-lock (`moonAnchored`, longitude locked, latitude identity); Moon position-lock (same kind, both axes locked); Sun longitude-lock (`sunAnchored`, longitude locked, latitude identity); Sun position-lock (same kind, both axes locked). Moon and Sun share one axis-lock transform; they remain distinct kinds. Runtime only — not persisted, not `viewMode`, not URL state, and not civil-time reference. `SceneCamera` is unchanged (`scale`, `centerU`, `centerV`). Anchored frames do not write the Moon or Sun into `centerU` / `centerV`. The continuous longitude anchor follows `src/core/longitudeContinuity.ts` (Moon from `sublunarPoint`, Sun from `subsolarPoint`, same nearest-equivalent policy). Latitude has no continuity state: it is the current sublunar or subsolar latitude. Horizontal camera wrapping is not longitude continuity. Sign convention: `sceneLon = nearestEquivalent(canonicalLon, λAnchor_continuous) − λAnchor_continuous`; positive scene longitude is east of the anchor. Longitude-lock leaves `sceneLat = canonicalLat`. Position-lock uses `sceneLat = canonicalLat − anchorLat`. Scene-frame latitude is not periodic and may leave geographic ±90°; projection maps it linearly. Inverse latitude adds the anchor and clamps only when producing canonical geographic latitude. Sun longitude-lock’s scene origin is the current **subsolar meridian**, not civil clock noon.
