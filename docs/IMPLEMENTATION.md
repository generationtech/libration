...
| Solar eclipses | `solarEclipseLayer.ts` | NASA-derived solar overlay (E1 live footprint + E2 forecast corridor + E5 alignment beam + E6 labels/styles + live ground-position marker). Default **on**: geography appears only while an event is relevant. Forecast horizon (`0` / live only, `1`, `3`, `7`, `14`, `30`, `90`, `365` days; default **7**) is how early upcoming solar geography appears, not eclipse duration. Upcoming events emit an event-path corridor (cached, time-independent) and a representative greatest-eclipse partial region. While the event is globally active the corridor remains as path context (fill ~80% of upcoming strength; limits stay strong) even before/after the umbra is on Earth. The representative forecast partial region is upcoming-only; the live partial footprint owns active partial shading. Active events emit the live E1 umbra/antumbra and partial footprint when those geometries exist. A live ground-position marker sits on the authoritative central point while that intersection exists. An optional alignment ribbon (E5) connects the Sun/Moon glyph cluster to that live umbra/antumbra only while a terrestrial central target exists; partial-only events get a local bloom; central events with no current Earth intersection emit no beam. Partial-only events never fabricate a central corridor or marker. Presentation-only type filters (total / annular / partial / hybrid) default **on**. Child geography toggles and user style persist on the row. Alignment is `scene.eclipseAlignment`. Canvas sees no eclipse astronomy. |
...
- **Partial forecast:** representative **greatest-eclipse** penumbral outline, not the event-long swept penumbral union. It is drawn only while the event is upcoming. Once globally active, the live partial footprint owns current partial shading. Partial-only events show that forecast region before start, then the live partial region, and never a central corridor.
...
- **Visual families ([LIB-026](work/LIB-026-solar-eclipse-visual-semantics-reconciliation.md)):** six stable languages. **Event path** — static violet/lilac corridor (`rgba(72, 48, 140, …)` fill, lilac limits). **Live partial** — restrained teal-slate footprint (`rgba(47, 109, 120, 0.16)`), not path-violet and not night-blue. **Live central** — compact indigo umbra (`rgba(40, 24, 72, 0.50)`) or warm antumbra. **Alignment** — warm gold ribbon (narrow outer band, stronger core/axis). **Ground marker** — vermilion locator. **Ordinary night** — the illumination `rasterPatch` (cool near-black, unmodified). Upcoming forecast partial uses the live-partial hue at lower alpha and is absent once globally active.
...
- **Layering.** Within the solar eclipse overlay, generic `drawOrder` on equirect fills/strokes yields: forecast representative partial (upcoming only) → corridor fill → live partial footprint → corridor limit strokes → alignment bands → live umbra/antumbra → alignment axis → live/forecast centerline → ground-position marker → labels. Corridor fill stays below the live partial so the path is context; corridor limits stay above it so the route remains readable. Alignment bands draw after the live partial and before the compact central footprint. Glyphs stay above the eclipse layers. Ordinary solar/day-night shading is the illumination `rasterPatch` under the overlay and is not modified by solar-eclipse presentation. The solar ribbon is a directional beam (origin half-width ~5.4°, Dramatic ~6.4°), not a map-scale shading region.
