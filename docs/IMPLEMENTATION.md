...
| Lunar eclipses | `lunarEclipseLayer.ts`, Earth-shadow fields on `sublunarMarkerLayer.ts` | NASA-derived lunar overlay (E3 + E6 labels/styles + forecast window + LIB-021 spatial Moon shadow + LIB-043/044 presentation). Default **on**: geography appears only while an event is relevant. Separate lunar forecast horizon (`0` / live only, `1`, `3`, `7`, `14`, `30`, `90`, `365` days; default **7**). Upcoming and active events both emit the **current-instant** Moon-above-horizon region (dark informational fill, default `rgba(22, 34, 54, 0.12)`) and geometric horizon boundary around ambient `sublunarPoint` — not a freeze of greatest-eclipse geography and not a terrestrial path. Only the nearest upcoming lunar event gets map geography; after last contact the overlay disappears. Active events also receive spatial Earth-shadow Moon treatment. The lunar alignment control draws a short Moon-local Earth-shadow directional cue on the glyph (not a geographic map ribbon). Placard copy may still describe visibility at greatest eclipse as event information. Presentation-only type filters (total / partial / penumbral) default **on**. Child controls and user style persist on the row. Alignment is `scene.eclipseAlignment` (`lunarEnabled` stored boolean; UI copy is Earth-shadow cue). Lunar map labels use glyph-relative candidates at the current Moon and avoid city-name boxes when city pins are on. Canvas sees no eclipse astronomy. |
...
- **Config groups** in Layers: Eclipses (information / labels); Solar (horizon, types, live/forecast geography); Lunar (horizon, types, upcoming/active Moon-visible geography at the current product instant, Moon-shadow); Alignment; Reference city; Eclipse appearance. Child controls disable when the parent layer or master is off. Solar forecast corridor/partial disable when the solar horizon is Live only. Lunar upcoming region/boundary disable when the lunar horizon is Live only.
