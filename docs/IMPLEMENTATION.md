...
The application is usable with no network. All base-map rasters, the emissive night-lights raster, and the font assets are bundled and served from `public/`. Clouds/IR and earthquakes fall back to recorded fixtures when live acquisition fails. ISS hides when no live TLE can be acquired (CelesTrak primary, Where the ISS at secondary; fixture is tests/DEV only and is never painted as live). Nothing in the render path requires the network.
...
| Lunar eclipses | `lunarEclipseLayer.ts`, Earth-shadow fields on `sublunarMarkerLayer.ts` | NASA-derived lunar overlay (E3 + E6 labels/styles + forecast window + LIB-021 spatial Moon shadow + LIB-043/044/046 presentation). Default **on**: event presentation appears only while an event is relevant. Separate lunar forecast horizon (`0` / live only, `1`, `3`, `7`, `14`, `30`, `90`, `365` days; default **7**). Lunar eclipse map presentation does **not** paint a terrestrial Moon-visible hemisphere or geometric lunar-horizon boundary. The eclipse is communicated by the Moon glyph, Earth-shadow treatment, Moon-local Earth-shadow cue, physically attenuated moonlight, HUD/placard, and event label. Forecast horizon Live only hides upcoming events; it does not change physical moonlight. After last contact the event overlays disappear. Ordinary Moon-above-horizon astronomy remains in illumination (`lunarDot ≥ 0`), sublunar geometry, and local circumstances. Presentation-only type filters (total / partial / penumbral) default **on**. Child controls persist on the row. Alignment is `scene.eclipseAlignment` (`lunarEnabled` stored boolean; UI copy is Earth-shadow cue). Lunar map labels use glyph-relative candidates at the current Moon and avoid city-name boxes when city pins are on; they do not avoid an unpainted horizon line. Canvas sees no eclipse astronomy. |
...
bundled lunar authority JSON
  ...
  → Moon glyph earthShadowOverlay  +  lunar eclipse layer (labels only; no terrestrial visibility fill)
  → existing Moon RenderPlan / equirectRegionOverlay
  → Canvas (text; no lunar-eclipse region fill/stroke)
```
...
- **Moon-above-horizon astronomy:** geometric lunar altitude = 0 on a spherical Earth (same unit-sphere dot product as illumination `lunarDot`). No refraction. This geometry still gates physical moonlight (`lunarDot ≥ 0`) and local circumstances. It is **not** painted as a lunar-eclipse map overlay ([LIB-046](work/LIB-046-remove-lunar-eclipse-moon-visible-geography.md)). Removed: eclipse-specific Moon-visible presentation (hemisphere fill and white geometric-horizon stroke). Preserved: ordinary astronomical Moon visibility mechanics. **Placard:** the information panel may still say where the Moon is above the horizon at greatest eclipse (`lunar-forecast-visibility-v1` remains as event-information geometry). The map legend does not describe unpainted geography. The reference city never selects global eclipse presentation.
- **Wrap:** solar eclipse overlays still use generic `equirectRegionOverlay` polar-close / world-copy path construction. Lunar eclipse map presentation no longer emits a visibility ring, so lunar wrap copies of that overlay are gone. Horizon-ring helpers remain for forecast/event-information geometry and illumination tests.
...
- **HUD copy.** Persistent eclipse status answers what the event means at the reference city. Solar HUD percent is **obscuration** (Sun area covered), not magnitude: integer below 99%; one decimal from 99% to &lt;100% that never rounds a partial to `"100%"`; `"100%"` only when the value is truly 1.0. Upcoming with local C1: `begins {time}`. Upcoming without C1: keep relative `in 50m`. Active before local max: `max {time}`. After local max with C4: `ends {time}`. Not-visible copy never hides the global event.
...
Acquisition is periodic and runs on an injectable timer, never inside `requestAnimationFrame`, a layer constructor, or a RenderPlan builder. Refresh cadence is per source: 15 minutes for clouds/IR, 5 minutes for earthquakes, 2 hours for the ISS TLE (CelesTrak GP update cycle). Enabling a consumer calls `startPeriodic` with `runImmediately: true`, so the first fetch does not wait for the interval tick. ISS re-enable with a still-ready in-memory snapshot starts the interval without an immediate re-download. ISS TLE refresh is background maintenance; the marker moves every frame via local SGP4(product UTC). Clouds/IR and earthquakes do not pass an HTTP timeout.
...
Three live feeds are wired, each classified `timePolicy: "wallClockCurrent"` under their present implementations. Clouds/IR and earthquakes keep a recorded real-format fixture as offline fallback under the same durable `sourceId`. ISS does not: if both live TLE providers fail with no usable live TLE, the overlay is unavailable. ISS HTTP attempts are bounded at 8 seconds; timeout is not treated as a user abort, so the secondary provider can run in the same cycle. After all-provider failure, a single 5-minute retry is scheduled if ISS remains enabled, then the 2-hour cadence. [ADR 0014](decisions/0014-iss-live-tle-ordered-provider-failover.md).
...
2. **Visibility (Galactic-center altitude contours).** How high the Galactic center is above the geometric horizon from each terrestrial location. Each contour is a small circle around the same GC subpoint as the marker, radius `90° − h`. Factory **off** even when the master is on. When the user enables contours: 30/45/60/75° on, horizon 0° off, astronomical-night emphasis on (stronger where Sun ≤ −18°, smooth through twilight), moonlight de-emphasis on (existing phase × incidence × eclipse transmission; does not erase geometry). One visibility color/thickness; one small `30°` / `45°` / `60°` / `75°` label per contour. Not a brightness, seeing, or light-pollution score, and not an aggregated observing-quality forecast.
3. **Viewing windows (reference-city events).** Headless `listMilkyWayViewingWindows` in `milkyWayViewingWindows.ts` builds bounded UTC intervals for the chrome reference city ([ADR 0018](decisions/0018-milky-way-viewing-window-is-a-reference-city-event.md), policy `milky-way-viewing-v1`). Viewing: GC ≥ 15° and Sun ≤ −12°. Strong: GC ≥ 20° and ≥ 75% of local nightly max, Sun ≤ −18°, moonlight ≤ 0.22. Prime: ≥ 90% of local nightly max (and GC ≥ 15°), Sun ≤ −18°, moonlight ≤ 0.08. Nightly max is `90° − |lat − GC Dec|`. Relative Prime applies only when that max is at least 15°. Peak maximizes GC altitude. Factory **Enable Milky Way viewing events** off; when on, Layers Viewing/Strong/Prime checkboxes control **map event labels**, not Data playback. Independent of the overlay master, ribbon, and contours. Map labels (upcoming/active, default 2-day advance notice) anchor at the Galactic-center subpoint with reference-city copy. Time navigation is Data → Event playback, not Layers. Clouds and light pollution are not inputs. Eclipse HUD is unchanged.
...
| Tab | Owns |
|-----|------|
| Layers | Scene stack toggles (Layer masters topic, default): lunar ground track, lunar locus, Solar eclipses, Lunar eclipses, and the other overlay masters. Internal **Layers topic** selector (UI-only, not persisted; inactive topics unmount; compact selector uses shared `.config-topic-nav` `position: sticky` inside the existing `.config-tab-panel` scroller, not the viewport; changing topic resets that panel’s `scrollTop` and does not call `updateConfig`): Map (family, preview, attribution, presentation); Illumination (moonlight, night lights, cloud participation); Eclipse System (event information / labels; solar and lunar forecast horizons and type filters; Moon Earth-shadow; alignment; reference-city details/chrome; independent appearance; active solar eclipse shading enable and Subtle/Normal/Dramatic intensity — presentation only, no event tour); Moon & libration (size, ring/crosshair, map/observer orientation, use-reference-city, color, thickness, motion scale); Astronomy paths (past/future track extents and stroke colors; independent Lunar locus and Solar analemma stroke color/thickness); **Space objects** (ISS presentation: orbit track, past/future segments, minute/orbit horizons, colors, thickness, glyph, size, label; **Planets**: Mercury–Neptune plus Pluto body enable/color/locus plus shared current-subpoint, glyph, ground-track, and locus style; **Milky Way**: zenith-projection Galactic plane / approximate band / ribs / Galactic center, GC altitude contours, viewing-event enable and map-label presentation — not product-time navigation); Advanced (overlay-readability presentation and per-layer pilots). Optional live overlays (clouds/IR, earthquakes, ISS) remain Layer masters checkboxes. Planets and Milky Way are also Layer masters (offline astronomy, factory off). Live eclipse event rows live on the map, not in this tab. |
...
| Data | Internal **Data topic** selector (UI-only, not persisted; same sticky `.config-topic-nav` as Layers/Chrome). Default **Time**: pipeline mode and generic Demo-time controls. **Event playback**: Eclipse and Milky Way viewing-window sequencing through the shared Demo clock ([ADR 0019](decisions/0019-domain-event-playback-belongs-to-data.md)). Data owns when the product is viewed; Layers own what is rendered. |
...
Event playback is a domain sequencer **above** Demo time, not a second clock ([ADR 0015](decisions/0015-domain-tour-sequencer-drives-shared-demo-time.md), [ADR 0019](decisions/0019-domain-event-playback-belongs-to-data.md)). Controls live under **Data → Event playback**. Layers keep presentation only.

Durable `data.eventPlayback` stores selected family plus per-family range, filters, loop, lead-in, and post-wait. Pre-LIB-052 `scene.eclipseTour` migrates into `data.eventPlayback.eclipse` and is omitted from normalized scene output. Playback speed is `data.demoTime.speedMultiplier` only. Runtime phase, event index, and owned Demo start ISO are session-only and start inactive.

On Start, the selected family enumerates events, sets Demo mode/enabled/start to the current event’s clamped lead-in, and resumes Demo playback. At each event’s clamped post-wait it jumps Demo start to the next lead-in. Loop wraps; loop off pauses at the final allowed instant. Stop deactivates sequencing and leaves product time paused. Reset returns to the **current event** lead-in (which is also the Demo configured start while a sequence is active), not the range start. Generic Data → Time Reset uses that same configured Demo start, so during an active sequence it is equivalent to Event Playback Reset. **Use current time** (and any other manual Demo start edit) changes `startIsoUtc` away from the owned instant and deactivates sequencing; product time stays at the user-selected instant.

Eclipse playback preserves LIB-047 catalog times, Solar/Lunar family toggles, and presentation-coupled subtype filters. Layer masters do not gate enumeration. Milky Way playback uses the chrome reference city, Data-owned Viewing/Strong/Prime eligibility (factory Viewing off, Strong/Prime on), and grouped nightly opportunities so Viewing→Strong→Prime→Strong→Viewing plays as one event. Changing reference city or family while running deactivates playback.
