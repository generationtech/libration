...
The **scene/map reference frame** (`src/core/sceneReferenceFrame.ts`) is a transform *before* projection, carried on `SceneRenderInput.sceneReferenceFrame`. Production kinds: Earth-fixed identity (default; exact short-circuit so LIB-081 numbers are unchanged); Moon longitude-lock (`moonAnchored`, longitude locked, latitude identity); Moon position-lock (same kind, both axes locked). Runtime only — not persisted, not `viewMode`, not URL state, and not civil-time reference. `SceneCamera` is unchanged (`scale`, `centerU`, `centerV`). Moon frames do not write the Moon into `centerU` / `centerV`. The continuous lunar longitude anchor follows `src/core/longitudeContinuity.ts`. Latitude has no continuity state: it is the current sublunar latitude. Horizontal camera wrapping is not longitude continuity. Sign convention: `sceneLon = nearestEquivalent(canonicalLon, λMoon_continuous) − λMoon_continuous`; positive scene longitude is east of the Moon. Longitude-lock leaves `sceneLat = canonicalLat`. Position-lock uses `sceneLat = canonicalLat − moonAnchorLat`. Scene-frame latitude is not periodic and may leave geographic ±90°; projection maps it linearly. Inverse latitude adds the anchor and clamps only when producing canonical geographic latitude.

Earth-fixed geographic rasters keep their LIB-081 dest mapping. Under Moon longitude-lock the same full-world equirectangular strip is shifted by `−λMoon_continuous / 360 × width` and copied with the existing periodic dest machinery so base map, illumination, and Clouds stay registered. Position-lock adds a vertical dest shift of `−moonAnchorLat / 180 × height` with **no** vertical copies: the translated Earth is clipped, not wrapped. Tiles and raster reprojection are not part of this layer.

Shared mapping: `sceneXFromLongitudeDeg` / `sceneYFromLatitudeDeg` compose frame → projection → camera (optional frame argument defaults to Earth-fixed). The Canvas backend threads the live frame into plan builders. A compact **Scene frame** control (Earth-fixed / Moon — longitude locked / Moon — position locked) sits with Reset view; switching any of those three recenters the camera to identity; Reset view resets the camera only. Reload returns to Earth-fixed. Architecture: [`docs/specs/scene/camera-and-reference-frame.md`](specs/scene/camera-and-reference-frame.md), [ADR 0026](decisions/0026-scene-camera-independent-of-projection-and-reference-frame.md), [ADR 0027](decisions/0027-moon-longitude-lock-is-a-scene-reference-frame.md), [ADR 0028](decisions/0028-moon-position-lock-translates-scene-frame-latitude.md).
