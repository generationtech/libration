...
- **Layering.** Within the solar eclipse overlay: forecast representative partial (upcoming only) → corridor fill/limits → live partial → alignment bands → live umbra/antumbra → alignment axis → live/forecast centerline → ground-position marker → labels. Alignment bands draw after the live partial region and before the live umbra/antumbra, so the footprint remains the geographic target. Glyphs stay above the eclipse layers. Moon-over-Sun overlap is unchanged. Ordinary solar/day-night shading is the illumination `rasterPatch` under the overlay and is not modified by solar-eclipse presentation.
