...
| Lunar eclipses | `lunarEclipseLayer.ts`, Earth-shadow fields on `sublunarMarkerLayer.ts` | NASA-derived lunar overlay (E3 + E6 labels/styles + forecast window + LIB-021 spatial Moon shadow + LIB-043/044/045 presentation). Default **on**: geography appears only while an event is relevant. Separate lunar forecast horizon (`0` / live only, `1`, `3`, `7`, `14`, `30`, `90`, `365` days; default **7**). One Moon-visible region control and one Moon-visible boundary control own the **current-instant** Moon-above-horizon fill (default `rgba(22, 34, 54, 0.12)`) and geometric horizon stroke around ambient `sublunarPoint` for both upcoming and active presentation — not a freeze of greatest-eclipse geography, not a terrestrial path, and not separate lifecycle paint gates. Only the nearest upcoming lunar event gets map geography; after last contact the overlay disappears. Forecast horizon Live only hides upcoming events but does not disable the visibility controls; they still apply while an event is active. Active events also receive spatial Earth-shadow Moon treatment. The lunar alignment control draws a short Moon-local Earth-shadow directional cue on the glyph (not a geographic map ribbon). Placard copy may still describe visibility at greatest eclipse as event information. Presentation-only type filters (total / partial / penumbral) default **on**. Child controls and user style persist on the row. Alignment is `scene.eclipseAlignment` (`lunarEnabled` stored boolean; UI copy is Earth-shadow cue). Lunar map labels use glyph-relative candidates at the current Moon and avoid city-name boxes when city pins are on. Canvas sees no eclipse astronomy. |
...
- **Visibility region:** geometric lunar altitude = 0 on a spherical Earth (same unit-sphere dot product as illumination `lunarDot`). No refraction. **Map semantics (upcoming and active):** the great-circle horizon contour around the current ambient `sublunarPoint` — locations where the Moon is geometrically above the local horizon **at the current product instant**. The fill is a dark informational overlay (`rgba(22, 34, 54, 0.12)` default), not a moonlight lift and not a second terrestrial shade field. There is no lifecycle switch from a frozen greatest-eclipse hemisphere to live geography; that switch was a playback discontinuity (LIB-044). **Placard forecast:** the information panel may still say where the Moon is above the horizon at greatest eclipse (`lunar-forecast-visibility-v1` remains as event-information geometry). Equatorial moons use a 180° longitude strip sampled so dateline unwrap does not invert the hemisphere. The reference city never selects or recenters this region.
- **Wrap:** the same generic `equirectRegionOverlay` / polar-close path as solar. Hemisphere rings (circular lon span > 270°) keep sequential unwrap so the cut does not jump among near-equal sample gaps as the Moon longitude crawls. A near-equatorial Moon-up hemisphere is allowed to span the map in longitude; an equatorial strip crossing ±180° is split via world copies, not one 360° rectangle.
...
- **Config groups** in Layers: Eclipses (information / labels); Solar (horizon, types, live/forecast geography); Lunar (horizon, types, Moon Earth-shadow, unified Moon-visible region/boundary at the current product instant); Alignment; Reference city; Eclipse appearance. Child controls disable when the parent layer or master is off. Solar forecast corridor/partial disable when the solar horizon is Live only. Lunar Moon-visible region/boundary stay available whenever Lunar eclipses is enabled, including when the lunar horizon is Live only.
