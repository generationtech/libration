...
The application is usable with no network. All base-map rasters, the emissive night-lights raster, and the font assets are bundled and served from `public/`. Clouds/IR fall back to a recorded fixture when live acquisition fails. Earthquakes and ISS hide when their live sources are unavailable; they never present fixture data as live. Nothing in the render path requires the network.
...
Three live feeds are wired, each classified `timePolicy: "wallClockCurrent"` under their present implementations. Clouds/IR keep a recorded real-format fixture as offline fallback under the same durable `sourceId`. Earthquakes and ISS do not: a failed live fetch with no usable live snapshot leaves the overlay unavailable. ISS HTTP attempts are bounded at 8 seconds; timeout is not treated as a user abort, so the secondary provider can run in the same cycle. After all-provider failure, a single 5-minute retry is scheduled if ISS remains enabled, then the 2-hour cadence. [ADR 0014](decisions/0014-iss-live-tle-ordered-provider-failover.md).
...
Failure policy is `stale-when-cached` for all three live consumers. Clouds/IR still fall back to fixture on empty cache. Earthquakes and ISS do not: first-ever live failure with an empty cache is unavailable (no markers). A later earthquake poll failure keeps the prior **live** snapshot while snapshot age is ≤ 60 min (status stale, not live); snapshot age > 60 min suppresses markers as unavailable. Snapshot age is `productUtcMs − acquiredAtMs` (how old the USGS copy is), distinct from per-event age (`productUtcMs − properties.time`). Aborts do not trigger fixture fallback. Earthquake Layers copy: loading / live / stale / unavailable / DEV fixture; historical live-only copy still wins.

**Earthquakes presentation.** Live authority remains USGS `all_day.geojson` (5 min cadence, immediate fetch on enable). Layers → **Earthquakes** owns local filters over that snapshot: minimum magnitude (factory 2.5+), maximum event age (factory 24 hours), Earthquakes only (factory on; excludes USGS types other than `earthquake`), Show labels (factory on), and label minimum magnitude (factory 4.0+). Changing those parameters rebuilds the overlay immediately because `sceneRuntimeAffectingEqual` includes `dynamicPointFeatures.source.parameters`; no refetch. Markers stay magnitude-scaled orange discs; labels use a compact `M4.6 · place` formatter rather than the provider `title`. Null magnitude is hidden unless minimum magnitude is All. Event timestamps up to 2 min in the future count as age 0. Depth, clustering, selection, tsunami/alert symbology, and earthquake Event Playback are not implemented.
