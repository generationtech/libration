...
The **scene/map reference frame** (`src/core/sceneReferenceFrame.ts`) is a transform *before* projection, carried on `SceneRenderInput.sceneReferenceFrame`. Production kinds: Earth-fixed identity (default; exact short-circuit so LIB-081 numbers are unchanged) and Moon longitude-lock (`moonAnchored`, longitude locked, latitude identity). Runtime only — not persisted, not `viewMode`, not URL state, and not civil-time reference. `SceneCamera` is unchanged (`scale`, `centerU`, `centerV`). Moon longitude-lock does not write the Moon into `centerU`. The continuous lunar anchor follows `src/core/longitudeContinuity.ts` (canonical, wrapped delta, nearest equivalent, continuous follow, 360° rebase). Horizontal camera wrapping is not that continuity. Sign convention: `sceneLon = nearestEquivalent(canonicalLon, λMoon_continuous) − λMoon_continuous`; positive scene longitude is east of the Moon. Latitude remains canonical.

Earth-fixed geographic rasters keep their LIB-081 dest mapping. Under Moon longitude-lock the same full-world equirectangular strip is shifted by `−λMoon_continuous / 360 × width` and copied with the existing periodic dest machinery so base map, illumination, and Clouds stay registered. Tiles and raster reprojection are not part of this layer.

Shared mapping: `sceneXFromLongitudeDeg` / `sceneYFromLatitudeDeg` compose frame → projection → camera (optional frame argument defaults to Earth-fixed). The Canvas backend threads the live frame into plan builders. A compact **Scene frame** control (Earth-fixed / Moon — longitude locked) sits with Reset view; switching frames recenters the camera to identity; Reset view resets the camera only. Reload returns to Earth-fixed. Architecture: [`docs/specs/scene/camera-and-reference-frame.md`](specs/scene/camera-and-reference-frame.md), [ADR 0026](decisions/0026-scene-camera-independent-of-projection-and-reference-frame.md), [ADR 0027](decisions/0027-moon-longitude-lock-is-a-scene-reference-frame.md).
...
- [`docs/specs/scene/camera-and-reference-frame.md`](specs/scene/camera-and-reference-frame.md) — scene camera and map reference frame; zoom (LIB-080), pan (LIB-081), Earth-fixed identity (LIB-082), and Moon longitude-lock (LIB-083) are implemented. Sun-fixed and Moon latitude lock remain unscoped.
