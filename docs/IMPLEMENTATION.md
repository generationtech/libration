...
| Solar eclipses | `solarEclipseLayer.ts` | NASA-derived solar overlay (E1 live footprint + E2 forecast corridor + E5 alignment beam). Default **off**. Forecast horizon (`0` / live-only, `1`, `3`, `7`, `14`, `30`, `90`, `365` days; default **7**) selects upcoming events at product UTC. Upcoming events emit a quieter event-path corridor (cached, time-independent) and a representative greatest-eclipse partial region. Active events emit the live E1 umbra/antumbra and partial footprint, and may retain the corridor as context when the horizon is not live-only. Partial-only events never fabricate a central corridor. An optional active-only alignment ribbon (E5) connects the Sun/Moon glyph cluster to the live umbra/antumbra; partial-only events get a local bloom, not a fabricated target. Presentation: `showCentralLine` / `showCentralBand` / `showPartialRegion` (live) and `showForecastCorridor` / `showForecastPartialRegion` (forecast); all default **on**. Alignment is `scene.eclipseAlignment`, independently disableable. Canvas sees no eclipse astronomy. |
| Lunar eclipses | `lunarEclipseLayer.ts`, Earth-shadow fields on `sublunarMarkerLayer.ts` | NASA-derived lunar overlay (E3 + E5 axis). Default **off**. Active events only: no lunar forecast UI. The layer emits the terrestrial Moon-above-horizon region and geometric horizon boundary, plus an optional Earth-shadow alignment axis toward the Moon glyph. Earth-shadow treatment on the Moon glyph is a clipped overlay inside the existing Moon RenderPlan (`earthShadowOverlay`), after phase shading and before the libration mark. Child controls (Moon eclipse-shadow, visibility boundary, visibility region) default **on**. Alignment is `scene.eclipseAlignment`. Canvas sees no eclipse astronomy. |
...
- [`docs/specs/scene/eclipse-system.md`](specs/scene/eclipse-system.md) — Eclipse System architecture; E1–E5 are production, E6 remains unapproved.
