...
| Lunar eclipses | `lunarEclipseLayer.ts`, Earth-shadow fields on `sublunarMarkerLayer.ts` | NASA-derived lunar overlay (E3 + E6 labels/styles + forecast + LIB-021 spatial Moon shadow + LIB-043 presentation). Default **on**: geography appears only while an event is relevant. Separate lunar forecast horizon (`0` / live only, `1`, `3`, `7`, `14`, `30`, `90`, `365` days; default **7**). Upcoming events emit a quieter representative Moon-visible region at **greatest eclipse** (cached, not a terrestrial path). Only the nearest upcoming lunar event gets map geography. Active events emit the **current-instant** Moon-above-horizon region (dark informational fill, default `rgba(22, 34, 54, 0.12)`), geometric horizon boundary, and spatial Earth-shadow Moon treatment. The lunar alignment control draws a short Moon-local Earth-shadow directional cue on the glyph (not a geographic map ribbon). Forecast effects disappear at event start. Presentation-only type filters (total / partial / penumbral) default **on**. Child controls and user style persist on the row. Alignment is `scene.eclipseAlignment` (`lunarEnabled` stored boolean; UI copy is Earth-shadow cue). Lunar map labels use glyph-relative candidates and avoid city-name boxes when city pins are on. Canvas sees no eclipse astronomy. |
