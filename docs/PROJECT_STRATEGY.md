...
The next strategic need is not another large hidden architecture migration. The **planetary composition baseline** (eight-intrinsic substrate lift, third twilight pass, overlay readability closure) is **complete** for standing incremental work—**Slice 2 queues B/C are closed** as default PR tracks. Phase 10 lifecycle is **complete**. Default engineering pull is post–Phase 10 **`DLC-*`** ([`docs/specs/scene/dynamic-data-lifecycle-plan.md`](specs/scene/dynamic-data-lifecycle-plan.md); next **`DLC-1`**). **Remaining Phase 8** and **Phase 9** stay **deferred** unless explicitly scoped. Composition extensions reopen only with **explicit product scope**—not filler PRs.
...
2. **Dynamic layer consumers** (**default macro PR track:** `DLC-*` in [`docs/specs/scene/dynamic-data-lifecycle-plan.md`](specs/scene/dynamic-data-lifecycle-plan.md); next **`DLC-1`** global equirect clouds/IR). Phase 10 lifecycle **complete**. Weather participation models: [`docs/specs/scene/weather-cloud-composition-plan.md`](specs/scene/weather-cloud-composition-plan.md).
