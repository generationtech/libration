...
4. **Readability and composition extensions** (`perLayer` beyond the **six shipped** default-stack keys where product defines new rows; **further** substrate/catalog signals beyond `reliefShaded` / `boundaryDense` / `chromaticDense` / `bathymetryShaded` + dimming; **further** atmospheric refinement on the **existing** upstream illumination stack **after** the shipped incremental twilight tuning pass), and cloud/weather **planning** until dynamic lifecycle opens.
