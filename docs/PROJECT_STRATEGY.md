...
2. Map inventory curation and selector polish (**default macro PR track:** Phase 8 / Slice 3 when a shippable gap exists; else Slice 2 **B**/**C**; **structured attribution presentation** and **month-aware selector polish** **shipped**; queue **(2)** land cover, bathymetry, and climate normals **`equirect-world-climate-koppen-beck-v1`** **shipped**; **single best next PR:** next sourced static substrate per `PLAN.md` handoff).
3. Further scientific substrate onboarding (next sourced families, emissive-compatible substrates) and map inventory polish beyond the shipped static scientific substrates (topography, political, geology, bathymetry, land cover, Köppen–Geiger climate).
