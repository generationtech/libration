...
- **Phase 8 / Slice 3 map inventory (rolling):** all **nine** bundled catalog families have `previewThumbnailSrc` (legacy + static scientific substrates + Blue Marble) and **structured attribution** in the selector (**Source & license** block; catalog `licenseNote` + `sourceLinks`)—**default next** queue **A** PR: **climate normals** substrate (`PLAN.md` Slice 3 item **(2)**).
...
- climate map families (**land cover shipped:** **`equirect-world-landcover-modis-v1`**).
