...
- **Phase 8 / Slice 3 map inventory (queue A (2) closed):** all **ten** bundled catalog families have `previewThumbnailSrc` (legacy + static scientific substrates + Blue Marble) and **structured attribution** in the selector (**Source & license** block; catalog `licenseNote` + `sourceLinks`)—**default next** **Slice 2 / queue B** unless a **sourced static substrate** re-opens queue **A** (`PLAN.md` handoff).
...
- **Phase 8 / Slice 3–4 attribution presentation (closed):** richer attribution in `BaseMapStyleControl` + catalog fields on all **ten** bundled families; see `PLAN.md` closed increment.
...
Status: planned (**in progress**; queue **A (2)** **closed** for current catalog). **Default next macro PR track:** **Slice 2 / queue B** (ninth+ substrate intrinsic) per `PLAN.md` handoff—queue **A** re-opens when a **sourced static substrate** (raster + rights) or explicitly scoped catalog polish is in scope. **Structured attribution presentation** and **month-aware selector polish** **shipped** (queue **A** items **(1)** and **(2b)** closed). **Queue (2) shipped:** land cover **`equirect-world-landcover-modis-v1`**, bathymetry **`equirect-world-bathymetry-etopo-v1`**, climate normals **`equirect-world-climate-koppen-beck-v1`**. Queue **D** weather/cloud **planning** **shipped**—implementation blocked on Phase 10.
