...
- **Phase 8 / Slice 3 map inventory (rolling):** all **ten** bundled catalog families have `previewThumbnailSrc` (legacy + static scientific substrates + Blue Marble) and **structured attribution** in the selector (**Source & license** block; catalog `licenseNote` + `sourceLinks`)—**default next** queue **A** PR: next **sourced static substrate** when raster + rights exist, else Slice 2 **B**/**C** (`PLAN.md` handoff).
...
Status: planned (**in progress**). **Default next macro PR track** when `PLAN.md` **Agent session handoff** queue **A** has a shippable gap (map inventory before optional Slice 2 composition micro-tuning unless a session explicitly scopes Slice 2); otherwise **Slice 2** **B**/**C**. **Structured attribution presentation** in map selector is **shipped** (queue **A** item **(1)** closed). **Month-aware selector polish** is **shipped** (queue **A** item **(2b)** closed). **Queue (2) shipped:** land cover **`equirect-world-landcover-modis-v1`**, bathymetry **`equirect-world-bathymetry-etopo-v1`**, climate normals **`equirect-world-climate-koppen-beck-v1`**. **Single best next PR:** next **sourced static substrate** when raster + rights exist, else Slice 2 **B** then **C**. Queue **D** weather/cloud **planning** **shipped**—implementation blocked on Phase 10.
