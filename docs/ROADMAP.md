...
- **Phase 8 / Slice 3 map inventory (queue A (2) closed):** all **ten** bundled catalog families have `previewThumbnailSrc` (legacy + static scientific substrates + Blue Marble) and **structured attribution** in the selector (**Source & license** block; catalog `licenseNote` + `sourceLinks`)—**default next** **Phase 8 / queue A** when sourced substrate or explicit scope exists; **Slice 2 queues B/C closed** (`PLAN.md` handoff).
