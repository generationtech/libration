...
Case list: [`docs/specs/scene/camera-and-reference-frame.md`](specs/scene/camera-and-reference-frame.md) §12. Pan combinations are [LIB-081](work/LIB-081-scene-camera-pan.md) (complete): identity, 1× horizontal wrap, vertical clamp at 1×, zoom+pan, pointer-stable zoom after pan, Reset, dateline/eclipse/clouds/ISS/earthquake-hover wrap, resize while panned. [LIB-082](work/LIB-082-scene-reference-frame-foundation.md) is Earth-fixed identity: the same fixture set is a regression pass against completed LIB-081. [LIB-083](work/LIB-083-moon-longitude-locked-scene-frame.md) adds Moon longitude-lock: Earth-fixed regression first, then Moon-frame static/animated, lunar antimeridian, antipodal seam, camera independence, raster registration, tracks/eclipse/ISS/Clouds/earthquakes. [LIB-084](work/LIB-084-moon-position-locked-scene-frame.md) adds Moon position-lock: Earth-fixed and longitude-lock regression, then both-axis Moon origin, animated Earth vertical motion, standstill/extreme latitude, no vertical wrap, camera independence. [LIB-085](work/LIB-085-sun-anchored-scene-frames.md) adds Sun longitude-lock and Sun position-lock: Earth-fixed and Moon regression, then Sun origin, animated Earth motion, solar antimeridian, seasonal solar-latitude comparison, illumination registration, Moon-as-non-anchor, eclipses, Clouds, ISS, earthquakes, resize. [LIB-086](work/LIB-086-generalize-anchored-scene-reference-frames.md) is an architectural consolidation: the same five-mode matrix is a regression pass against completed LIB-085 (no new user-visible frame). [LIB-087](work/LIB-087-automatic-scene-cover-zoom-for-position-locked-frames.md) adds automatic scene-cover zoom on position-lock: no translation-caused top/bottom blank band; Moon/Sun stay frame-fixed; manual wheel override then Reset; Earth-fixed and longitude-lock remain identity; resize recalculates auto-cover but does not re-arm a manual override. [LIB-088](work/LIB-088-trackable-map-object-foundation.md) is an architectural foundation: the same five-mode matrix is a regression pass against completed LIB-087 (no new user-visible frame; Moon/Sun remain the only targets). [LIB-089](work/LIB-089-iss-tracking-target.md) adds ISS longitude-lock and ISS position-lock as a third `TrackableMapObjectId` on the same anchored frame: ISS stays on the centre meridian (longitude-lock) or at scene origin (position-lock); auto-cover uses generic latitude math; ISS options disable when no valid ISS position exists; `iss-presentation` is the primary ISS fixture. [LIB-090](work/LIB-090-tracking-target-and-mode-ux-foundation.md) replaces the seven-choice Scene frame selector with Tracking target + Tracking mode: Earth-fixed is no-target with mode disabled; Moon/Sun/ISS remain the only targets; mode is retained across target switches; ISS unavailable still disables ISS and falls back to Earth-fixed; camera/cover/Reset mathematics are unchanged. [LIB-091](work/LIB-091-direct-click-to-track-for-map-objects.md) adds click/tap on rendered Moon, Sun, and ISS glyphs as Target selection: remembered mode is retained; pan-vs-click uses the existing 4px drag threshold; wrapped copies share one id; empty geography does not clear tracking; earthquakes remain hover-only. [LIB-092](work/LIB-092-city-and-planet-tracking-targets.md) adds city pins and eligible planetary current glyphs (Mercury–Neptune plus Pluto when painted) on that same stack: grouped native Target `<select>`; structured identities; click-to-track via the LIB-091 hit seam; generic longitude/position lock and auto-cover; no city- or planet-specific frame or camera math. [LIB-093](work/LIB-093-galactic-center-and-anticenter-tracking-targets.md) adds Galactic Center and Galactic Anticenter as explicit Milky Way point targets on that same stack: identities from existing tagged geometry; no synthetic `"milkyWay"` target; the galactic-plane band is not trackable. `baseline` is the primary city fixture; `planetary-objects` is the primary planet fixture; `milky-way` is the primary galactic-point fixture (enable Galactic Anticenter in Layers for both points). Enable missing layers in-session for cross-category sequences. Reload returns to Earth-fixed. Reset view does not change the frame. In position-lock, Reset restores the automatic cover default (which may be `scale > 1`).
...
2. Open Layers → Weather. Factory Cloud opacity **0.42**. Status is an observation-age **range** of visible components only. Optional “Observation times” lists GOES-East/West, Meteosat, Himawari (and the ring only when it owns composed pixels). Attribution mentions EUMETSAT and NASA GIBS. No source selector or sync-mode control.
3. Confirm white/gray translucent cloud structures over Americas, Europe, Africa, Asia, Oceania, Atlantic, and Indian Ocean; polar holes stay transparent (base map visible, not black, not inferred clear sky). Clear ocean and warm land should not wear a broad pale veil; major frontal systems and deep convection should remain obvious. Some warm low cloud / thin cirrus may be weaker (intentional conservative default).
4. Opacity low (~0.15) / factory / high (~0.80): derived cloud alpha still multiplies; geography remains readable at factory **0.42**. Do not lower opacity to hide wash.
5. Day side and night side of the same composition: clouds remain visible without overpowering illumination. Night must not become a global gray-white wash merely because surfaces are cooler. No day/visible mode switch.
...
8. DEV-only source diagnostics: ordinary current time (not a `?scenario=`). `?cloudsSectorDebug=1` or `coverage` tints coverage footprints (paint-order, ignoring quality). `?cloudsSectorDebug=winner` tints the quality-aware selected source (good ring violet; poor ring darker). `?cloudsSectorDebug=ring` shows only ring-owned pixels (good vs poor). `?cloudsSectorDebug=q0ring` marks q=0 regional vs good/poor ring decisions (violet: good ring over q=0; orange: q=0 regional because ring is poor). `?cloudsSectorDebug=quality` tints the selected source with nadir-to-limb brightness. `?cloudsSectorDebug=ringQuality` visualizes inferred ring component-geometry quality as grayscale. `?cloudsSectorDebug=ringComponent` tints the inferred max-quality ring component (MSG / IODC / GOES-East / GOES-West / Himawari); this is diagnostic geometry, not EUMET per-pixel provenance. `?cloudsSectorDebug=signal` is derived cloud-confidence winners. `?cloudsSectorDebug=canonical` shows winner canonical display-IR as grayscale. `?cloudsSectorDebug=gibsGray` classifies GIBS pixels as near-gray warm-branch (cyan-gray) vs chromatic LUT (magenta). `?cloudsTransfer=legacy` (DEV only) reapplies the former Rec.601 path on the same observation slots for comparison. `?cloudsGibsGray=legacy|hybrid` (DEV only) compares WEATHER-5.1 RGB-nearest GIBS LUT vs chroma-aware warm-gray inversion on the same observations; the winner map must not change. `?cloudsRingCalibration=identity|bp56` (DEV only) compares production identity grayscale vs the former ring black-point 56 on the same observations; the winner map must not change. `production` aliases identity. `?cloudsSectorDebug=leak` marks pixels where selected clear coverage suppressed another source’s cloud. Production dist must not include these diagnostics.
9. Coverage vs ghost cloud: find a region where a selected **usable (q>0)** regional is valid-clear and the ring or an earlier regional would have been cloudy. The map must be **clear** there (older cloud must not show through). A nearby region where the selected regional is cloudy must still show cloud. Valid-clear **q=0** regional coverage yields to a **good** (q>0) ring when the ring has coverage; **poor** (q=0) ring yields back to that q=0 regional. q=0 regional still paints where the ring is absent. Polar ring α=0 stays no-data.
10. Quality-aware North Atlantic handoff: the conspicuous Meteosat western disk-edge arc must not be the cloud source transition where GOES-East has substantially better viewing geometry. GOES-East should own the western North Atlantic; Meteosat should take authority farther east where its view becomes competitive. After canonical IR, residual contrast at that geometric handoff should be weaker than the former GIBS-wash vs MSG-binary step. Cyclone structures must not look basin-wide blended. High-latitude cold surface may still resemble cloud (IR limitation). Ring must not intrude on that q>0 overlap.
11. India / southern Indian Ocean / Antarctic: usable q>0 regional disks remain circular. The ring corridor between them should not treat every provider-valid ring pixel as equal quality. IODC-viewed southern Indian Ocean is good ring; far-south poor ring may return to q=0 regional where regionals exist. Large pre-LIB-073 Meteosat WMS circular cuts must not return as the dominant envelope. Remaining ring-native polar scallops are provider-intrinsic and may remain. True polar no-data stays transparent. The India ring→Himawari θ=75° class edge remains the correct authority line; after chroma-aware GIBS near-gray inversion it must not be dominated by salt-and-pepper false cold cloud. Ordinary ring decks west of that line should be visible under identity grayscale, not trapped below the confidence floor. Compare `?cloudsRingCalibration=bp56` vs default identity on the same observations: ordinary-cloud mean step should shrink; Sahara / Arabian Sea / clear Atlantic must not wash; the 25°N chromatic convective-core step need not vanish. Antarctica may brighten existing cold-surface IR (IR-only limit, not a polar special case). Southern Indian Ocean ~70°E 45°S should not stay a zero-confidence dark corridor when other IR sources show cloud.
...
2. Open Layers → Earthquakes. Factory: Minimum magnitude 2.5+, Maximum age 24 hours, Earthquakes only on, Show earthquake labels on, Label minimum magnitude 4.0+, Show label on hover on. The map should read as a global earthquake map, not a wall of micro-event titles. M4+ events stay labeled; smaller visible markers are unlabeled until hovered.
3. Change Minimum magnitude and Label minimum magnitude: markers/labels update on the next frame with no USGS refetch.
4. Set Maximum age to 1 hour: only recent markers remain; no refetch. Hover only those remaining markers.
5. Show earthquake labels OFF, hover ON: marker-only map; hovering one visible marker shows one compact `M#.# · place` label; leaving the marker clears it. Hover OFF: pointer produces nothing.
6. Historical Demo (for example 2017-08-21) with Earthquakes still checked: live-only suppression copy; no markers; no hover; no fixture. Return to current: acquisition/reuse without re-checking the box.
