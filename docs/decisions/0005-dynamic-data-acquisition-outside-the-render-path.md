...
- Offline operation is first-class. Recorded real-format fixtures remain available for tests and DEV scenarios. Production Clouds, earthquakes, and ISS must not silently present fixture data as live; a failed live fetch with no usable live snapshot leaves those overlays unavailable.
- Failure policy is uniform (`stale-when-cached`: prefer the last good version over surfacing an error) rather than reinvented per source. Last-good **live** snapshots may still paint under a source-specific freshness policy; that is not fixture substitution. Clouds v1 freshness uses observation age (`validTimeMs`), not fetch time.
...
- Fixture fallback means an offline session can silently display recorded data **when an adapter opts in**. Production Clouds, earthquakes, and ISS hide instead. DEV `?scenario=clouds` may paint a labeled fixture. "Something is drawn" is not proof that a live GIBS fetch worked.
