...
- **Status:** Accepted
- **Date:** 2026-08-22 (record written with [LIB-069](../work/LIB-069-weather-4-3-quality-plane-overlap-authority.md); ring-over-q0 consequence amended with [LIB-073](../work/LIB-073-weather-5-2-ring-outranks-q0-regional-authority.md); composite/backstop quality-plane consequence amended with [LIB-075](../work/LIB-075-weather-5-3-1-ring-component-geometry-quality.md))
...
- Each contributing source keeps three independent planes: **coverage**, **quality**, and **signal**. Composite or backstop imagery that is assembled from multiple geostationary components also requires an independent quality plane derived from source geometry, not from provider alpha and not from image content.
- **Coverage** remains provider valid-data. Quality must not turn valid coverage into no-data. A quality=0 observation that is the only coverage at a pixel still paints. When a valid global backstop also covers that pixel, a geometrically usable (quality>0) backstop is preferred over quality=0 regional coverage; a geometrically unusable (quality=0) backstop yields to quality=0 regional coverage when that regional exists, and still paints when it is the only coverage. Any usable (quality>0) regional still beats the backstop, including valid-clear.
...
- Preferred authority when every covering regional is quality=0: a paintable global backstop with **usable** component geometry wins; otherwise a quality=0 regional still paints; a quality=0 backstop still fills true gaps. This does not redefine coverage.
...
- A valid q=0 regional observation may lose to a geometrically usable global backstop while remaining observational coverage. A geometrically unusable backstop pixel may lose to q=0 regional coverage without becoming a hole. Those are preferred-authority, not coverage holes.
- Quality planes are Earth-fixed and can be cached outside the render path. Backstop quality inferred from documented component sub-satellite geometry is an approximation when the provider does not expose per-pixel source provenance.
