...
- capabilities (including optional **overlay-readability** hints such as `reliefShaded`, `boundaryDense`, `chromaticDense`, `bathymetryShaded`, `fineScaleTexture`, and `labelDense`, alongside `overlayOptimized` and `darkFriendly`, for upstream overlay lift scaling without raster sampling).
- recommended roles.
