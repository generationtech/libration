...
anchored
    target                          TrackableMapObjectId   (moon | sun | iss | city | planet)
...
Targets may be **dynamic** (Moon, Sun, ISS, planets) or **static** (city pins). The frame does not require motion. Clickable object selection is implemented for Moon, Sun, ISS, city pins, and eligible planetary current glyphs. Earthquakes are not tracking targets. Milky Way is not a target: the overlay is a galactic-plane band plus tagged Galactic Center/Anticenter points, not one canonical tracking point.
...
**UI.** Compact runtime **Target** and **Mode** controls. Target is a native `<select>`: Earth-fixed ungrouped, then Celestial (Moon, Sun, eligible planets), Spacecraft (ISS), and Cities optgroups. Mode is Longitude / Position and is disabled under Earth-fixed. Clicking a visible Moon, Sun, ISS, city-pin, or current-planet glyph sets Tracking target through `setTrackingTarget`; remembered mode is retained ([LIB-091](../../work/LIB-091-direct-click-to-track-for-map-objects.md), [ADR 0035](../../decisions/0035-click-to-track-uses-scene-space-semantic-hit-targets.md), [LIB-092](../../work/LIB-092-city-and-planet-tracking-targets.md), [ADR 0036](../../decisions/0036-city-and-planet-tracking-reuses-structured-target-identity.md)). Native-select option values are UI encoding only.
...
A redesigned Tracking target + Tracking mode control is implemented in [LIB-090](../../work/LIB-090-tracking-target-and-mode-ux-foundation.md). Direct click-to-track for Moon, Sun, and ISS is implemented in [LIB-091](../../work/LIB-091-direct-click-to-track-for-map-objects.md). City pins and eligible planetary current glyphs are implemented in [LIB-092](../../work/LIB-092-city-and-planet-tracking-targets.md).
