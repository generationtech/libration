...
anchored
    target                          TrackableMapObjectId   (moon | sun | iss | city | planet | milkyWayPoint)
...
Moon, Sun, ISS, city pins, eligible planetary current glyphs, Galactic Center, and Galactic Anticenter are production **trackable map object** identities (`src/core/trackableMapObject.ts`). Named ids remain `"moon" | "sun" | "iss"`; cities, planets, and Milky Way points are structured `{ kind: "city", id }` / `{ kind: "planet", id }` / `{ kind: "milkyWayPoint", id }`. Forward/inverse transform, raster dest shift, camera vertical extent, longitude continuity, and automatic cover branch on Earth-fixed vs anchored and on `lockMode` — not on which object is the target. Target resolution is a separate seam: canonical instant + authoritative product state → canonical lon/lat, then the frame is built from those numbers. Moon uses the existing `sublunarPoint`; Sun uses the existing `subsolarPoint`; ISS uses the existing ISS current sample (`resolveIssCurrentSample`) when the overlay would itself paint (`issTrackShouldPaint`); a city uses the same `CityPinEntry` lon/lat already used to paint the pin (static); a planet uses the same current mapped subpoint already used to paint the glyph; Galactic Center and Anticenter use `geometry.galacticCenter` / `geometry.galacticAnticenter` from the same Milky Way payload used to paint those glyphs. Runtime policy: `src/core/sceneFrameAnchor.ts`. Selection UI: `src/core/trackingSelection.ts`. ISS availability: [ADR 0033](../../decisions/0033-iss-tracking-reuses-anchored-frame-target-architecture.md). Orthogonal target/mode UI: [ADR 0034](../../decisions/0034-tracking-ui-is-orthogonal-target-and-mode.md). Structured city/planet identity: [ADR 0036](../../decisions/0036-city-and-planet-tracking-reuses-structured-target-identity.md). Milky Way point tracking: [ADR 0037](../../decisions/0037-galactic-center-and-anticenter-tracking-reuses-existing-milky-way-points.md).
...
Targets may be **dynamic** (Moon, Sun, ISS, planets, Galactic Center, Galactic Anticenter) or **static** (city pins). The frame does not require motion. Clickable object selection is implemented for Moon, Sun, ISS, city pins, eligible planetary current glyphs, Galactic Center, and Galactic Anticenter. Earthquakes are not tracking targets. The galactic-plane band is not a target. There is no synthetic `"milkyWay"` identity.
...
**UI.** Compact runtime **Target** and **Mode** controls. Target is a native `<select>`: Earth-fixed ungrouped, then Celestial (Moon, Sun, eligible planets, Galactic Center, Galactic Anticenter when rendered), Spacecraft (ISS), and Cities optgroups. Mode is Longitude / Position and is disabled under Earth-fixed. Clicking a visible Moon, Sun, ISS, city-pin, current-planet, Galactic Center, or Galactic Anticenter glyph sets Tracking target through `setTrackingTarget`; remembered mode is retained ([LIB-091](../../work/LIB-091-direct-click-to-track-for-map-objects.md), [ADR 0035](../../decisions/0035-click-to-track-uses-scene-space-semantic-hit-targets.md), [LIB-092](../../work/LIB-092-city-and-planet-tracking-targets.md), [ADR 0036](../../decisions/0036-city-and-planet-tracking-reuses-structured-target-identity.md), [LIB-093](../../work/LIB-093-galactic-center-and-anticenter-tracking-targets.md), [ADR 0037](../../decisions/0037-galactic-center-and-anticenter-tracking-reuses-existing-milky-way-points.md)). Native-select option values are UI encoding only. The galactic-plane band is not click-to-track.
...
ISS, city, planet, Galactic Center, and Galactic Anticenter longitude-lock and position-lock are implemented ([LIB-089](../../work/LIB-089-iss-tracking-target.md)–[LIB-093](../../work/LIB-093-galactic-center-and-anticenter-tracking-targets.md), [ADR 0033](../../decisions/0033-iss-tracking-reuses-anchored-frame-target-architecture.md)–[ADR 0037](../../decisions/0037-galactic-center-and-anticenter-tracking-reuses-existing-milky-way-points.md)). A later target should reuse this identity + resolution + common lock-mode path rather than invent a special transform. There is no synthetic `"milkyWay"` target. Earthquakes remain excluded.
...
A redesigned Tracking target + Tracking mode control is implemented in [LIB-090](../../work/LIB-090-tracking-target-and-mode-ux-foundation.md). Direct click-to-track for Moon, Sun, and ISS is implemented in [LIB-091](../../work/LIB-091-direct-click-to-track-for-map-objects.md). City pins and eligible planetary current glyphs are implemented in [LIB-092](../../work/LIB-092-city-and-planet-tracking-targets.md). Galactic Center and Galactic Anticenter are implemented in [LIB-093](../../work/LIB-093-galactic-center-and-anticenter-tracking-targets.md).
...
### 12.8.4 Direct click-to-track (LIB-091)

Implemented. Interaction only. Production `SceneReferenceFrame` and camera/cover math are unchanged. Clicking a rendered Moon, Sun, or ISS glyph copy calls `setTrackingTarget` with that `TrackableMapObjectId` and retains remembered mode.

Automated: hit-target ids; scene-centre agreement with painted glyphs under Earth-fixed, longitude-lock, position-lock, zoom, pan, and wrap; wrapped copies share one id; hit radius inside/outside; nearest-centre overlap and stable tie order; Earth-fixed click Moon; cross-target click; same-target no-op; position-lock auto-cover; longitude-lock identity; ISS unavailable refuse; pan vs click; empty geography; earthquakes not collected; dropdown `setTrackingTarget` equivalence.

Visual: Earth-fixed → click Moon/Sun (Position remembered, auto-cover); `iss-presentation` click ISS in both modes; Moon → Sun → ISS clicks; same-target click no jump; pan starting on a glyph does not select; wrapped copy click; zoom/pan then click; pointer/grab/grabbing cursor; `baseline` ISS unclickable; earthquake click does not track; resize 1280×720.

### 12.8.5 City and planet tracking targets (LIB-092)

Implemented. Integration only. Production `SceneReferenceFrame` and camera/cover math are unchanged. Cities are static `CityPinEntry` identities; eligible planets are Mercury–Neptune plus Pluto when a current mapped glyph is painted. Earth is not a target. Both Target-control selection and click-to-track call `setTrackingTarget`. Native `<optgroup>` Target chrome; select values are UI encoding. Hit testing reuses the LIB-091 scene-space seam. Nearest centre wins; ties use category+identity (`trackableMapObjectIdTieKey`), not a hand-maintained global list. Overlapping city pins have no chooser. Earthquakes remain hover-only. Milky Way point tracking is [LIB-093](../../work/LIB-093-galactic-center-and-anticenter-tracking-targets.md).

Automated: distinct city/planet/Moon/Sun/ISS identities; semantic equality for reconstructed structured ids; native-select key round-trip; city resolution to existing pin lon/lat; planet resolution to the painted current subpoint; missing city/planet availability fallback; grouped options; city/planet longitude and position lock; static city time progression; planet continuity around ±180°; generic auto-cover from city/planet latitude; identical numeric anchors produce identical transforms/cover regardless of target category; city/planet hit targets and clicks; wrap copies; same-target no-op; nearest-centre overlap; earthquake exclusion; Moon/Sun/ISS regressions.

Visual: grouped Target control at 1920×1080 and 1280×720; city dropdown and click in Longitude and Position; static city vs moving Moon/Sun/ISS; planet dropdown and click; dynamic planet motion; auto-cover and manual override; pan; wrap copy; cross-category Moon → city → planet → ISS; earthquake click does not track.

### 12.8.6 Galactic Center and Anticenter tracking targets (LIB-093)

Implemented. Integration only. Production `SceneReferenceFrame` and camera/cover math are unchanged. Identities are structured `{ kind: "milkyWayPoint", id: "galacticCenter" | "galacticAnticenter" }`. There is no synthetic `"milkyWay"` target. Positions come from the existing Milky Way geometry payload used to paint the tagged glyphs (dynamic Earth-relative zenith subpoints). Availability is planet-style omit: listed and trackable only when that glyph is currently rendered (layer visible, payload supported, presentation flag on, finite lon/lat). Factory: Center on, Anticenter off. Both Target-control selection and click-to-track call `setTrackingTarget`. Celestial grouping appends Galactic Center and Anticenter after planets. Native-select values `milkyway:galacticCenter` / `milkyway:galacticAnticenter` are UI encoding only. Hit testing reuses the LIB-091 scene-space seam and the painted glyph radius; the galactic-plane band is not a hit target. Earthquakes remain hover-only.

Automated: distinct Center/Anticenter identities; semantic equality; native-select key round-trip; resolution equals payload lon/lat at the same instant; omit when disabled/hidden; Earth-fixed fallback; longitude and position lock; ±180° continuity via the generic continuous-anchor path; generic auto-cover equivalence with other targets at the same latitude; hit targets under Earth-fixed/anchored/zoom/pan/wrap; wrap-copy identity; click selection; same-target no-op; band non-selection; earthquake exclusion; Moon/Sun/ISS/city/planet regressions.

Visual: `milky-way` Target control shows Galactic Center (Anticenter after enabling the glyph); no generic Milky Way option; Center/Anticenter Longitude and Position; direct click of tagged points; band click does not track; Demo time frame-lock; cross-category Moon → Galactic Center → Jupiter → city → Galactic Anticenter → ISS → Sun; manual override then Reset; representative overlays; earthquake exclusion; 1280×720 grouping and chrome.
