...
**LIB-065 / LIB-067 / LIB-069 / LIB-071 / Clouds v3 production note.** Clouds remains **Model B** (informational overlay): per-sector GEO IR + EUMET ring backstop → provider display interpretation → canonical display IR → shared cloud-confidence → local white/gray overlay → one composed `imageBlit`. Freshness outranks temporal uniformity ([ADR 0023](../../decisions/0023-observational-composites-heterogeneous-observation-times.md)). Each source keeps **coverage**, **quality**, and **cloud signal** as independent planes ([ADR 0024](../../decisions/0024-observational-quality-distinct-from-coverage.md)). Provider display rasters are normalized before shared presentation ([ADR 0025](../../decisions/0025-heterogeneous-display-normalized-before-shared-presentation.md)). Valid-clear coverage replaces older cloud; extreme-limb quality may lose to a modestly older better view; the ring is a backstop only where regional coverage is absent. Physical **Model A** participation is forced off because display cloud confidence is not cloud optical depth. Current implementation truth is [`docs/IMPLEMENTATION.md`](../../IMPLEMENTATION.md). This document remains useful for weather products that do **not** yet exist — radar, precipitation, wind, pressure, polar LEO fill — because the architectural questions it answers apply to each of them.
