...
**LIB-065 / LIB-067 / LIB-069 / LIB-071 / LIB-073 / LIB-075 / LIB-077 / LIB-079 / Clouds v3 production note.** Clouds remains **Model B** (informational overlay): provider observations → independent observation/acquisition times → coverage → geometric quality → source authority → provider-specific canonical IR → shared cloud-confidence → one composed `imageBlit`. The current-observation IR overlay is the Clouds foundation; later weather products in this document are not unfinished WEATHER-5 work. Freshness outranks temporal uniformity ([ADR 0023](../../decisions/0023-observational-composites-heterogeneous-observation-times.md)) among usable (q>0) observations. Each source keeps **coverage**, **quality**, and **cloud signal** as independent planes ([ADR 0024](../../decisions/0024-observational-quality-distinct-from-coverage.md)), including the ring: ring coverage ≠ ring quality ≠ ring signal. Ring quality is inferred from documented component GEO geometry (IODC 45.5°E included) because provider WMS lacks per-pixel source provenance. Provider display rasters are normalized before shared presentation ([ADR 0025](../../decisions/0025-heterogeneous-display-normalized-before-shared-presentation.md)). GIBS Band13 chromatic pixels use the 64³ colormap LUT; near-gray pixels invert along the warm-gray legend so WMS-resampled gray cannot snap onto the cold gray branch. Meteosat and the EUMET ring share identity grayscale (`luma / 255`); the former ring black-point 56 is not production. Valid-clear **q>0** coverage replaces older cloud; extreme-limb q=0 regional coverage remains observational coverage but loses to a **good** (q>0) ring and beats a **poor** (q=0) ring. Physical **Model A** participation is forced off because display cloud confidence is not cloud optical depth. Current implementation truth is [`docs/IMPLEMENTATION.md`](../../IMPLEMENTATION.md). This document remains useful for weather products that do **not** yet exist — radar, precipitation, wind, pressure, polar LEO fill — because the architectural questions it answers apply to each of them.
