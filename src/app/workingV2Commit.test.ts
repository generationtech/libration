...
describe("commitWorkingV2Update", () => {
  ...
  it("sceneRuntimeAffectingEqual is false when only overlay readability grid pilot veil changes", () => {
    const base = buildDefaultSceneConfigFromLayerFlags({
      baseMap: true,
      solarShading: true,
      grid: true,
  ...
  it("sceneRuntimeAffectingEqual is false when only overlay readability solarAnalemma pilot veil changes", () => {
    const base = buildDefaultSceneConfigFromLayerFlags({
      baseMap: true,
      solarShading: true,
      grid: true,
    ...
    const b: typeof base = {
      ...
      overlayReadability: {
        ...base.overlayReadability,
        perLayer: {
          solarAnalemma: { readabilityVeilScale01: 0.5, overlayLiftMultiplier01: 1 },
        },
      },
    };
    expect(sceneRuntimeAffectingEqual(base, b)).toBe(false);
