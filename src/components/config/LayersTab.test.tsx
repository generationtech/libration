...
describe("LayersTab topic navigation", () => {
  ...
  it("Milky Way viewing-event controls are independent of the overlay master", async () => {
    ...
    const footprint = screen.getByLabelText("Show viewing footprint") as HTMLInputElement;
    expect(master.checked).toBe(false);
    expect(events.checked).toBe(false);
    expect(events.disabled).toBe(false);
    expect(labels.disabled).toBe(true);
    expect(footprint.disabled).toBe(true);
    expect(screen.queryByLabelText("Show Viewing windows")).toBeNull();
    expect(screen.queryByLabelText("Show Strong windows")).toBeNull();
    expect(screen.queryByLabelText("Show Prime windows")).toBeNull();
    ...
    expect(labels.checked).toBe(true);
    expect(footprint.disabled).toBe(false);
    expect(footprint.checked).toBe(true);
