...
import { type TopBandAnchorConfig, type TopBandTimeMode } from "../../config/appConfig";
...
function defaultAnchorForMode(
  ...
  if (mode === "fixedCity") {
    const cityId = previous.mode === "fixedCity" ? previous.cityId : DEFAULT_FIXED_CITY_ID;
    return { mode: "fixedCity", cityId };
  }
  const longitudeDeg =
    previous.mode === "fixedLongitude" ? clampLongitudeDegForAnchor(previous.longitudeDeg) : 0;
  return { mode: "fixedLongitude", longitudeDeg };
}

export type ReferenceClockEditorProps = {
  config: LibrationConfigV2;
  updateConfig?: (updater: (draft: LibrationConfigV2) => void) => void;
};

export function ReferenceClockEditor({ config, updateConfig }: ReferenceClockEditorProps) {
  ...
  return (
    <div data-testid="chrome-editor-reference-and-clock">
              ...
              ? (e) => {
                  ...
                  updateConfig((draft) => {
                    if (source === "system") {
                      ...
                      const zone =
                        prev.source === "fixed" ? prev.timeZone : DEFAULT_FIXED_ZONE_WHEN_ENABLING;
