...
const CLIMATE_JPG = path.join(REPO_ROOT, "public/maps/world-equirectangular-climate.jpg");

/** Locks the Beck Köppen–Geiger present export; update when intentionally replacing raster bytes. */
const CLIMATE_SHA256_HEX =
  "8ad6eb5d13624345a02852daa675987c1cffd38d387318a14c0e7663ceaaf027";
...
describe("onboarded Köppen–Geiger climate normals base-map raster", () => {
  it("is a JPEG with SOF-reported 5400×2700 (exact 2:1 equirect)", () => {
    const bytes = readFileSync(CLIMATE_JPG);
  ...
  it("matches the locked SHA-256 of the shipped climate normals asset", () => {
    const bytes = readFileSync(CLIMATE_JPG);
    const digest = createHash("sha256").update(bytes).digest("hex");
    expect(digest).toBe(CLIMATE_SHA256_HEX);
  });

  it("registers longitude −180…+180 (Amazon Af blue, Sahara BWh red, Antarctica EF gray)", () => {
    const bytes = readFileSync(CLIMATE_JPG);
    ...
    expect(amazon.blueMinusRed).toBeGreaterThan(200);
    expect(amazon.meanB).toBeGreaterThan(200);
    ...
    expect(sahara.meanR).toBeGreaterThan(200);
    expect(sahara.meanG).toBeLessThan(20);
    expect(sahara.meanB).toBeLessThan(20);

    const antarctica = patchStatsAt(
      data,
      width,
      height,
      xFromLongitudeDeg(0, width),
      yFromLatitudeDeg(-75, height),
    );
    expect(antarctica.meanR).toBeGreaterThan(90);
    expect(antarctica.meanR).toBeLessThan(115);
    expect(Math.abs(antarctica.greenMinusRed)).toBeLessThan(5);
  });
});
