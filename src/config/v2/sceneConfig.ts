...
import {
  ...
  DEFAULT_LUNAR_VISIBILITY_FOOTPRINT_THICKNESS,
...
import {
  ...
  DEFAULT_SOLAR_ECLIPSE_ACTIVE_SHADING_INTENSITY,
  normalizeSolarEclipsePresentation,
} from "../../core/eclipse/solarEclipseAppearance";
import {
  DEFAULT_REFERENCE_CITY_ECLIPSE_CHROME_ENABLED,
  DEFAULT_REFERENCE_CITY_ECLIPSE_DETAILS_ENABLED,
  normalizeReferenceCityEclipsePresentation,
...
export type SceneConfig = {
  ...
  /**
   * Observer-derived eclipse circumstances presentation (E4). Independent of
...
const SOLAR_ECLIPSE_ROW: SceneLayerInstance = {
  ...
  source: {
    ...
    parameters: {
      ...
      showLiveGroundPosition: DEFAULT_SOLAR_ECLIPSE_SHOW_LIVE_GROUND_POSITION,
      activeEclipseShadingEnabled: DEFAULT_SOLAR_ECLIPSE_ACTIVE_SHADING_ENABLED,
      activeEclipseShadingIntensity: DEFAULT_SOLAR_ECLIPSE_ACTIVE_SHADING_INTENSITY,
    },
  },
};

/** NASA-derived lunar eclipse overlay: Moon Earth-shadow, labels, and forecast window. Default on. */
const LUNAR_ECLIPSE_ROW: SceneLayerInstance = {
  ...
  source: {
    ...
    parameters: {
      ...
      visibilityFootprintColor: DEFAULT_LUNAR_VISIBILITY_FOOTPRINT_COLOR,
      visibilityFootprintThickness: DEFAULT_LUNAR_VISIBILITY_FOOTPRINT_THICKNESS,
    },
  },
};
...
export function referenceCityEclipsePresentationFromScene(
  scene: SceneConfig,
): ReferenceCityEclipsePresentation {
  return normalizeReferenceCityEclipsePresentation(scene.eclipseCircumstances);
}

export function applyReferenceCityEclipsePresentationToScene(
  scene: SceneConfig,
  patch: Partial<ReferenceCityEclipsePresentation>,
): SceneConfig {
  const current = referenceCityEclipsePresentationFromScene(scene);
  return {
    ...scene,
    eclipseCircumstances: normalizeReferenceCityEclipsePresentation({ ...current, ...patch }),
  };
}
...
function omitRemovedLunarEclipsePresentationKeys(
  ...
  const {
    ...
    visibilityBoundaryColor: _boundaryColor,
    visibilityBoundaryThickness: _boundaryThickness,
    visibilityRegionColor: _regionColor,
    visibilityRegionOpacity: _regionOpacity,
    ...rest
  } = parameters;
  return rest;
}
...
function withNormalizedLunarEclipseParameters(source: LayerSourceConfig): LayerSourceConfig {
  ...
  return {
    ...source,
    parameters: {
      ...omitRemovedLunarEclipsePresentationKeys(source.parameters),
      ...p,
    },
  };
}
