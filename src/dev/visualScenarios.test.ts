...
describe("visual scenario query parsing", () => {
  ...
  it("ordinary cloudsSectorDebug=ring and q0ring are not visual scenarios", () => {
    expect(parseVisualScenarioQuery("?cloudsSectorDebug=ring")).toBeNull();
    expect(applyVisualScenarioFromLocation("?cloudsSectorDebug=ring").kind).toBe(
      "inactive",
    );
    expect(getDevCloudsSectorDebugTint()).not.toBeNull();
    expect(parseVisualScenarioQuery("?cloudsSectorDebug=q0ring")).toBeNull();
    expect(applyVisualScenarioFromLocation("?cloudsSectorDebug=q0ring").kind).toBe(
      "inactive",
    );
    expect(getDevCloudsSectorDebugTint()).not.toBeNull();
  });

  it("ordinary cloudsSectorDebug=ringQuality and ringComponent are not visual scenarios", () => {
    expect(parseVisualScenarioQuery("?cloudsSectorDebug=ringQuality")).toBeNull();
    expect(
      applyVisualScenarioFromLocation("?cloudsSectorDebug=ringQuality").kind,
    ).toBe("inactive");
    expect(getDevCloudsSectorDebugTint()).not.toBeNull();
    expect(parseVisualScenarioQuery("?cloudsSectorDebug=ringComponent")).toBeNull();
    expect(
      applyVisualScenarioFromLocation("?cloudsSectorDebug=ringComponent").kind,
    ).toBe("inactive");
    expect(getDevCloudsSectorDebugTint()).not.toBeNull();
  });
